...
Order/session/tab core product status is tracked in `docs/ORDER_SESSION_TAB_CORE.md`. Active order scoping by `tableSessionId`/`tabId`, Guest History and Post-Visit Feedback are closed and stay in regression. Force-close policy, personal-tab integrity, persistent repeat, preorder and broader analytics remain open; current Nth Hookah loyalty code is a substantial but not yet product/release-reconciled foundation, not a blank future feature.
...
Menu/options/stop-list product status is tracked in `docs/MENU_OPTIONS_STOPLIST.md`. Selected-option parity, Staff availability, Shift Check Phase 1, shared bootstrap and multiple bounded mutation-audit families are smoke-closed. Descriptions, featured/top-list, advanced constructor/modifiers, media and the exact remaining concurrency/RBAC/audit gaps stay open under the master inventory.
...
Booking lifecycle product status is tracked in `docs/BOOKING_LIFECYCLE.md`. Queue/hold/list/chat, confirmed arrival and booking `SEATED` → Guest History/Feedback paths are implemented by slice; reminder rollout, no-show semantics, conversation uniqueness, real two-account isolation and preorder remain open.

Telegram fallback/staff-chat product status is tracked in `docs/TELEGRAM_FALLBACK_STAFF_CHAT.md`. Staff-chat is radar/shortcut only. The bounded Platform Owner Guest-QR test escape is smoke-closed; Platform menu parity still needs an exact audit, and real fallback/staff-chat pilots, personal notification policy and delivery history remain open.
...
Guest growth/retention product status is tracked in `docs/GROWTH_RETENTION.md`. History, Post-Visit Feedback, venue favorites, simple promotions and Happy Hours are closed by bounded releases; Repeat Phase 1 is locally validated with deferred manual smoke, and Gift still needs release gates. Favorite-item and placement backend/Telegram foundations exist but need Mini App/product parity; Nth Hookah loyalty has substantial runtime that requires a dedicated product/release audit. Consent, campaigns, promo codes, referrals, paid-ad productization and advanced recommendations remain open/deferred.
