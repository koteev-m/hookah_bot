CREATE TABLE IF NOT EXISTS visit_feedback_requests (
    ...
    CONSTRAINT fk_visit_feedback_requests_visit FOREIGN KEY (visit_id) REFERENCES visits(id) ON DELETE CASCADE,
    CONSTRAINT fk_visit_feedback_requests_venue FOREIGN KEY (venue_id) REFERENCES venues(id) ON DELETE CASCADE,
    CONSTRAINT fk_visit_feedback_requests_user FOREIGN KEY (user_id) REFERENCES users(telegram_user_id) ON DELETE RESTRICT,
    CONSTRAINT chk_visit_feedback_requests_status CHECK (status IN ('PENDING', 'SENT', 'CANCELED', 'SKIPPED', 'FAILED'))
);
...
CREATE TABLE IF NOT EXISTS visit_feedback (
    ...
    comment TEXT NULL,
    status VARCHAR(32) NOT NULL,
    staff_notified_at TIMESTAMP WITH TIME ZONE NULL,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_visit_feedback_visit FOREIGN KEY (visit_id) REFERENCES visits(id) ON DELETE CASCADE,
    CONSTRAINT fk_visit_feedback_venue FOREIGN KEY (venue_id) REFERENCES venues(id) ON DELETE CASCADE,
    CONSTRAINT fk_visit_feedback_user FOREIGN KEY (user_id) REFERENCES users(telegram_user_id) ON DELETE RESTRICT,
    CONSTRAINT uq_visit_feedback_visit_user UNIQUE (visit_id, user_id),
    CONSTRAINT chk_visit_feedback_rating CHECK (rating IS NULL OR rating BETWEEN 1 AND 5),
    CONSTRAINT chk_visit_feedback_status CHECK (status IN ('SUBMITTED', 'SKIPPED'))
);
