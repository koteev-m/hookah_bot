ALTER TABLE IF EXISTS venue_settings
    ADD COLUMN IF NOT EXISTS public_review_url VARCHAR NULL;

CREATE TABLE IF NOT EXISTS guest_public_review_cta (
    ...
    CONSTRAINT fk_guest_public_review_cta_user FOREIGN KEY (user_id) REFERENCES users(telegram_user_id) ON DELETE CASCADE,
    CONSTRAINT fk_guest_public_review_cta_venue FOREIGN KEY (venue_id) REFERENCES venues(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_guest_public_review_cta_user
    ON guest_public_review_cta (user_id);

CREATE INDEX IF NOT EXISTS idx_guest_public_review_cta_venue
    ON guest_public_review_cta (venue_id);
