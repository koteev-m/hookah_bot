ALTER TABLE users
    ADD COLUMN IF NOT EXISTS guest_display_name CLOB NULL;

ALTER TABLE users
    ADD COLUMN IF NOT EXISTS birthday_month SMALLINT NULL;

ALTER TABLE users
    ADD COLUMN IF NOT EXISTS birthday_day SMALLINT NULL;

ALTER TABLE users
    ADD COLUMN IF NOT EXISTS birthday_set_at TIMESTAMP WITH TIME ZONE NULL;

ALTER TABLE users
    DROP CONSTRAINT IF EXISTS users_birthday_month_check;

ALTER TABLE users
    ADD CONSTRAINT users_birthday_month_check
        CHECK (birthday_month IS NULL OR birthday_month BETWEEN 1 AND 12);

ALTER TABLE users
    DROP CONSTRAINT IF EXISTS users_birthday_day_check;

ALTER TABLE users
    ADD CONSTRAINT users_birthday_day_check
        CHECK (birthday_day IS NULL OR birthday_day BETWEEN 1 AND 31);

ALTER TABLE users
    DROP CONSTRAINT IF EXISTS users_birthday_month_day_pair_check;

ALTER TABLE users
    ADD CONSTRAINT users_birthday_month_day_pair_check
        CHECK (
