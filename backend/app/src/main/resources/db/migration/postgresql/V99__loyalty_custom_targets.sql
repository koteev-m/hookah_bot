...
BEGIN
    FOR constraint_record IN
        ...
        WHERE conrelid = 'loyalty_programs'::regclass
          AND contype = 'c'
          AND pg_get_constraintdef(oid) ILIKE '%nth_value%'
    LOOP
        EXECUTE format('ALTER TABLE loyalty_programs DROP CONSTRAINT %I', constraint_record.conname);
    END LOOP;
END $$;

ALTER TABLE loyalty_programs
    ADD CONSTRAINT loyalty_programs_nth_value_range_check
...
BEGIN
    FOR constraint_record IN
        SELECT conname
        FROM pg_constraint
        WHERE conrelid = 'loyalty_program_earn_targets'::regclass
          AND contype = 'c'
          AND (
              pg_get_constraintdef(oid) ILIKE '%target_type%'
              OR pg_get_constraintdef(oid) ILIKE '%semantic_type%'
          )
    LOOP
        EXECUTE format('ALTER TABLE loyalty_program_earn_targets DROP CONSTRAINT %I', constraint_record.conname);
    END LOOP;
END $$;
...
ALTER TABLE loyalty_program_earn_targets
    ADD CONSTRAINT loyalty_program_earn_targets_target_type_check
    CHECK (target_type IN ('CATEGORY_TYPE', 'MENU_ITEM'));

ALTER TABLE loyalty_program_earn_targets
    ADD CONSTRAINT loyalty_program_earn_targets_semantic_type_check
    CHECK (semantic_type IS NULL OR semantic_type IN ('HOOKAH'));

ALTER TABLE loyalty_program_earn_targets
    ADD CONSTRAINT loyalty_program_earn_targets_shape_check
    CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_loyalty_program_earn_targets_category
    ON loyalty_program_earn_targets (program_id, semantic_type)
    WHERE target_type = 'CATEGORY_TYPE';

CREATE UNIQUE INDEX IF NOT EXISTS uq_loyalty_program_earn_targets_menu_item
    ON loyalty_program_earn_targets (program_id, menu_item_id)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_loyalty_program_reward_targets_category
    ON loyalty_program_reward_targets (program_id, semantic_type)
    WHERE target_type = 'CATEGORY_TYPE';

CREATE UNIQUE INDEX IF NOT EXISTS uq_loyalty_program_reward_targets_menu_item
    ON loyalty_program_reward_targets (program_id, menu_item_id)
    WHERE target_type = 'MENU_ITEM';
