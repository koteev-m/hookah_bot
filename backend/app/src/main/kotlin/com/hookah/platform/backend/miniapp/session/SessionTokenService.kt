...
data class SessionTokenConfig(
    ...
    companion object {
        fun from(
            ...
            val issuer =
                appConfig.propertyOrNull("api.session.issuer")
                    ?.getString()
                    ?.takeIf { it.isNotBlank() }
                    ?: "hookah-platform"
            val audience =
                appConfig.propertyOrNull("api.session.audience")
                    ?.getString()
                    ?.takeIf { it.isNotBlank() }
                    ?: "hookah-miniapp"
        ...
        private fun resolveJwtSecret(
            appConfig: ApplicationConfig,
            appEnv: String,
        ): String {
            val fromConfig =
                appConfig.propertyOrNull("api.session.jwtSecret")
                    ?.getString()
                    ?.trim()
                    ?.takeIf { it.isNotBlank() }
            val fromEnv =
                System.getenv("API_SESSION_JWT_SECRET")
                    ?.trim()
                    ?.takeIf { it.isNotBlank() }
        ...
        private fun resolveTtlSeconds(appConfig: ApplicationConfig): Long {
            val fromConfig =
                appConfig.propertyOrNull("api.session.ttlSeconds")
                    ?.getString()
                    ?.trim()
                    ?.takeIf { it.isNotBlank() }
            val fromEnv =
                System.getenv("API_SESSION_TTL_SECONDS")
                    ?.trim()
                    ?.takeIf { it.isNotBlank() }
