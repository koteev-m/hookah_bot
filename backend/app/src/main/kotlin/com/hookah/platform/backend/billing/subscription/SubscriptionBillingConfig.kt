...
data class SubscriptionBillingConfig(
    ...
    companion object {
        fun from(config: ApplicationConfig): SubscriptionBillingConfig {
            ...
            val leadDays = if (leadDaysParsed < 0) {
                logger.warn(
            ...
            val reminderDays = if (reminderDaysParsed < 0) {
                logger.warn(
            ...
            val intervalSeconds = if (intervalSecondsRaw.isNullOrBlank()) {
                60L
            } else {
                ...
                when {
                    parsedInterval == null -> {
                        logger.warn(
                            "billing.subscription.intervalSeconds is invalid ({}), subscription billing job disabled",
                    ...
                    parsedInterval <= 0 -> {
                        logger.warn(
                            "billing.subscription.intervalSeconds must be positive ({}), subscription billing job disabled",
                            sanitizeTelegramForLog(intervalSecondsRaw)
                        )
                        0L
                    }
