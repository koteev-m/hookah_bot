...
class BillingRepositoryTest {
    ...
    @Test
    fun `createInvoice is idempotent for same period`() =
        runBlocking {
            val venueId =
                dataSource.connection.use { connection ->
    ...
    @Test
    fun `insertPaymentEventIdempotent ignores duplicate events`() =
        runBlocking {
            val venueId =
                dataSource.connection.use { connection ->
                    insertVenue(connection)
    ...
    @Test
    fun `markInvoicePaid updates status and paid_at`() =
        runBlocking {
            val venueId =
                dataSource.connection.use { connection ->
                    insertVenue(connection)
