...
class BillingServiceTest {
    companion object {
        ...
        private fun invoiceStatusFor(connection: Connection, invoiceId: Long): Pair<String, Instant?> {
            return connection.prepareStatement(
                """
    ...
    @Test
    fun `applyPaymentEvent reconciles paid invoice even if payment event already stored`() = runBlocking {
        val venueId = dataSource.connection.use { connection ->
            insertVenue(connection)
        }
        val invoiceRepository = BillingInvoiceRepository(dataSource)
    ...
    @Test
    fun `applyPaymentEvent does not mark paid when amount or currency mismatch`() = runBlocking {
        val venueId = dataSource.connection.use { connection ->
            insertVenue(connection)
        }
        val invoiceRepository = BillingInvoiceRepository(dataSource)
    ...
    @Test
    fun `createDraftOrOpenInvoice is idempotent for provider calls`() = runBlocking {
        val venueId = dataSource.connection.use { connection ->
            insertVenue(connection)
        }
        val invoiceRepository = BillingInvoiceRepository(dataSource)
        ...
        val first = service.createDraftOrOpenInvoice(
            venueId = venueId,
            amountMinor = 6000,
            currency = "RUB",
        ...
        val second = service.createDraftOrOpenInvoice(
            venueId = venueId,
            amountMinor = 6000,
            currency = "RUB",
            description = "July subscription",
