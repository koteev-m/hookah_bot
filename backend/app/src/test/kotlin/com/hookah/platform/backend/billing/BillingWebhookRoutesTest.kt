...
class BillingWebhookRoutesTest {
    ...
    @Test
    fun `duplicate webhook delivery is idempotent`() = testApplication {
        val dbName = "billing_webhook_${UUID.randomUUID()}"
        val dataSource = HikariDataSource(
            HikariConfig().apply {
                driverClassName = "org.h2.Driver"
                jdbcUrl =
                    "jdbc:h2:mem:$dbName;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DEFAULT_NULL_ORDERING=HIGH;DB_CLOSE_DELAY=-1"
                maximumPoolSize = 3
            }
        )
        Flyway.configure()
