...
class GuestStaffCallRoutesTest {
    ...
    @Test
    fun `unauthorized staff call returns 401`() = testApplication {
        ...
        val response = client.post("/api/guest/staff-call") {
    ...
    @Test
    fun `happy path staff call`() = testApplication {
    ...
    @Test
    fun `invalid reason rejects staff call without resolver`() = testApplication {
        var resolveCalls = 0
        val config = MapApplicationConfig(
            "app.env" to appEnv,
            "api.session.jwtSecret" to "test-secret",
    ...
    @Test
    fun `invalid comment rejects staff call without resolver`() = testApplication {
        var resolveCalls = 0
        val config = MapApplicationConfig(
            "app.env" to appEnv,
            "api.session.jwtSecret" to "test-secret",
    ...
    @Test
    fun `suspended venue rejects staff call`() = testApplication {
    ...
    @Test
    fun `blocked subscription rejects staff call`() = testApplication {
    ...
    private fun fetchStaffCall(jdbcUrl: String, staffCallId: Long): StaffCallRecord? {
        DriverManager.getConnection(jdbcUrl, "sa", "").use { connection ->
            connection.prepareStatement(
                """
