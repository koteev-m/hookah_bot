...
class PlatformRoutesTest {
    ...
    @Test
    fun `non owner cannot access platform me`() =
        testApplication {
    ...
    @Test
    fun `non owner cannot list platform users`() =
        testApplication {
    ...
    @Test
    fun `owner can list platform users`() =
        testApplication {
            ...
            val token = issueToken(config, userId = ownerId)
            val response =
                client.get("/api/platform/users?limit=10") {
                    headers { append(HttpHeaders.Authorization, "Bearer $token") }
                }
