...
class PlatformSubscriptionRoutesTest {
    ...
    @Test
    fun `non owner cannot access subscription endpoints`() = testApplication {
        ...
        val getResponse = client.get("/api/platform/venues/$venueId/subscription") {
        ...
        val putResponse = client.put("/api/platform/venues/$venueId/subscription") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(PlatformSubscriptionSettingsUpdateRequest.serializer(), PlatformSubscriptionSettingsUpdateRequest()))
        }

        assertEquals(HttpStatusCode.Forbidden, putResponse.status)
        assertApiErrorEnvelope(putResponse, ApiErrorCodes.FORBIDDEN)

        val scheduleResponse = client.put("/api/platform/venues/$venueId/price-schedule") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(
                json.encodeToString(
    ...
    @Test
    fun `rejects invalid trial and paid dates`() = testApplication {
        ...
        val response = client.put("/api/platform/venues/$venueId/subscription") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(
                json.encodeToString(
    ...
    @Test
    fun `rejects invalid pricing`() = testApplication {
        ...
        val settingsResponse = client.put("/api/platform/venues/$venueId/subscription") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(
                json.encodeToString(
    ...
    @Test
    fun `effective price prefers override then schedule then base`() = testApplication {
        ...
        val baseResponse = client.put("/api/platform/venues/$venueId/subscription") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(
                json.encodeToString(
        ...
        val overrideResponse = client.put("/api/platform/venues/$venueId/subscription") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(
                json.encodeToString(
    ...
    private suspend fun getSubscription(
        ...
        val response = client.get("/api/platform/venues/$venueId/subscription") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
        }
