...
class VisitRepositoryTest {
    ...
    fun `guest visit promotion totals include only active adjustment snapshots`() =
        runBlocking {
            ...
            assertNotNull(detail)
            val historyOrder = detail.orders.single()
            assertEquals(2, historyOrder.items.size)
            assertEquals(1, historyOrder.items.count { !it.isExcluded })
            assertEquals(1, historyOrder.items.count { it.isExcluded })
            assertEquals(0L, historyOrder.items.single { it.isExcluded }.totalMinor)
            assertEquals(800L, historyOrder.totalMinor)
            val promotion = historyOrder.promotionDiscounts.single()
