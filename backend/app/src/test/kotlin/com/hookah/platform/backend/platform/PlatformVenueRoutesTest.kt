...
class PlatformVenueRoutesTest {
    ...
    @Test
    fun `assign owner is idempotent and unlocks publish`() = testApplication {
        ...
        val assignResponse = client.post("/api/platform/venues/$venueId/owners") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(PlatformOwnerAssignRequest.serializer(), PlatformOwnerAssignRequest(userId = 8111L)))
        }

        assertEquals(HttpStatusCode.OK, assignResponse.status)
        val assignPayload = json.decodeFromString(PlatformOwnerAssignResponse.serializer(), assignResponse.bodyAsText())
        assertEquals(true, assignPayload.ok)
        assertEquals(false, assignPayload.alreadyMember)

        val repeatResponse = client.post("/api/platform/venues/$venueId/owners") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(PlatformOwnerAssignRequest.serializer(), PlatformOwnerAssignRequest(userId = 8111L)))
        }
        ...
        val repeatPayload = json.decodeFromString(PlatformOwnerAssignResponse.serializer(), repeatResponse.bodyAsText())
        assertEquals(true, repeatPayload.alreadyMember)

        val publishResponse = client.post("/api/platform/venues/$venueId/status") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody("""{"action":"publish"}""")
        }
    ...
    @Test
    fun `owner invite creates invite and accept makes owner`() = testApplication {
        ...
        val inviteResponse = client.post("/api/platform/venues/$venueId/owner-invite") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(PlatformOwnerInviteRequest.serializer(), PlatformOwnerInviteRequest(ttlSeconds = 120)))
        }

        assertEquals(HttpStatusCode.OK, inviteResponse.status)
        val invitePayload = json.decodeFromString(PlatformOwnerInviteResponse.serializer(), inviteResponse.bodyAsText())
        ...
        val acceptResponse = client.post("/api/venue/staff/invites/accept") {
            ...
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(StaffInviteAcceptRequest.serializer(), StaffInviteAcceptRequest(inviteCode = invitePayload.code)))
        }
