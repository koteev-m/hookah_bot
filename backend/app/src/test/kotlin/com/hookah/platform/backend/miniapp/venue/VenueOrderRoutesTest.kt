...
class VenueOrderRoutesTest {
    ...
    @Test
    fun `manager can update status and audit is recorded`() = testApplication {
        ...
        val response = client.post("/api/venue/orders/$orderId/status?venueId=$venueId") {
            headers {
        ...
        val auditResponse = client.get("/api/venue/orders/$orderId/audit?venueId=$venueId") {
    ...
    @Test
    fun `invalid status transition returns error`() = testApplication {
    ...
    @Test
    fun `staff cannot reject orders`() = testApplication {
        ...
        val response = client.post("/api/venue/orders/$orderId/reject?venueId=$venueId") {
            headers {
                append(HttpHeaders.Authorization, "Bearer $token")
                append(HttpHeaders.ContentType, ContentType.Application.Json.toString())
            }
    ...
    @Test
    fun `queue pagination returns newest first`() = testApplication {
        ...
        val firstPage = client.get("/api/venue/orders/queue?venueId=$venueId&status=new&limit=1") {
            headers { append(HttpHeaders.Authorization, "Bearer $token") }
        }
    ...
    private fun seedMenu(jdbcUrl: String, venueId: Long) {
        DriverManager.getConnection(jdbcUrl, "sa", "").use { connection ->
    ...
    private fun seedOrder(jdbcUrl: String, venueId: Long, tableId: Long, status: String): Long {
        DriverManager.getConnection(jdbcUrl, "sa", "").use { connection ->
            return connection.prepareStatement(
                """
    ...
    private fun seedBatch(jdbcUrl: String, orderId: Long, status: String, createdAt: Instant): Long {
        DriverManager.getConnection(jdbcUrl, "sa", "").use { connection ->
            return connection.prepareStatement(
                """
    ...
    private fun seedBatchItem(jdbcUrl: String, batchId: Long) {
        DriverManager.getConnection(jdbcUrl, "sa", "").use { connection ->
            val menuItemId = connection.prepareStatement(
