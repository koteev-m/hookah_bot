...
usage() {
  cat <<'EOF'
Usage:
  scripts/v126-cutover.sh prepare-target-binding --help
  scripts/v126-cutover.sh bind-legacy-target --help
  scripts/v126-cutover.sh copy-target-binding-completion --help
...
try:
    ...
    if launch is not None:
        ...
        result = None
        if identity['kind'] == 'TARGET_BIND':
            proposal = request['genesis_request']
            binding_genesis_request(proposal, identity, Path(target))
            if request['mode'] == 'GENESIS_READBACK':
                result = binding_genesis_readback(target, identity, request['request_sha256'], policy_b_gate=gate)
            else:
                result = binding_genesis(target, identity, proposal, policy_b_gate=gate)
            status = 0
        elif request['mode'] == 'INIT_READBACK':
