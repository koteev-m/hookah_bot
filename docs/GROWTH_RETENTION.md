...
- Promotion effective state clarity is **PROMOTION EFFECTIVE STATE CLARITY / DONE / MVP / STAGING-SMOKE-PASSED**. The database lifecycle status is not rewritten automatically. The management UI derives `DRAFT`, `PAUSED`, `ARCHIVED`, `SCHEDULED`, `ACTIVE_NOW` and `EXPIRED` from authoritative lifecycle status and period only for presentation; `DRAFT`, `PAUSED` and `ARCHIVED` take precedence over time. `ACTIVE` before start is `Запланирована`, inside the inclusive period is `Действует сейчас`, and after end is `Период завершён`. An expired `ACTIVE` stays in `Текущие`, is hidden from Guest and excluded from promotion pricing, and offers extension/edit/archive without an automatic lifecycle mutation.
- `Продлить период` reuses the existing update plus authoritative reload. No automatic pause/archive, lifecycle worker, system actor or lifecycle audit was added. Live open-screen refresh at a time boundary, invalid-timestamp fail-safe UI, exact-boundary regression fixtures and the duplicate `Продлить период` / `Редактировать` product decision remain future.
...
- Promotion creation audit is **DANGEROUS ACTION AUDIT SLICE / PROMOTION CREATION AUDIT / DONE / MVP / STAGING-SMOKE-PASSED**. Every successful Mini App or Telegram parent creation routes through one repository transaction and appends exactly one `VENUE_PROMOTION_CREATED` for entity `venue_promotion` before commit.
- Parent, Mini App `afterInsert` initial Happy Hours/Gift rule and creation audit share one JDBC connection. Actor and `VENUE_MINI_APP` / `TELEGRAM_BOT` source are server-derived. Informational promotion creation uses `rules=[]`; Mini App Happy Hours/Gift includes the actually created initial rule; Telegram Happy Hours/Gift parent-draft creation correctly uses `rules=[]`. The safe payload contains only `venueId`, `promotionId`, `templateType`, `DRAFT` status, source and deterministic rule id/version/status rows. Denial, validation, `afterInsert`, SQL and audit failure create no success audit; audit failure rolls back parent and initial rules. Payload excludes promotion text/config/prices/media, raw Telegram payload/identity data and unrelated PII. Banner media persists separately. Configuration edit, schedule/target/reward, media/banner audit, Banner retry duplicate-draft UX and broader dangerous-action audit remain future.
...
- Promotion creation audit: `DANGEROUS ACTION AUDIT SLICE / PROMOTION CREATION AUDIT / DONE / MVP / STAGING-SMOKE-PASSED`; this closes only committed parent creation audit, not configuration edits, media or broader dangerous-action audit.
- Promotion effective state clarity: `PROMOTION EFFECTIVE STATE CLARITY / DONE / MVP / STAGING-SMOKE-PASSED`; lifecycle status remains authoritative and is not rewritten by time.
...
Recently closed bounded frontend block: **PROMOTION EFFECTIVE STATE CLARITY / DONE / MVP / STAGING-SMOKE-PASSED**. One Mini App helper derives the displayed badge, current-tab group, explanatory copy and available card actions from the authoritative lifecycle status plus `startsAt`/`endsAt`. `DRAFT`, `PAUSED` and `ARCHIVED` take precedence over time; an expired `ACTIVE` remains editable in `Текущие`, hidden from Guest and ineligible for pricing, and never triggers an automatic status write, audit or background process. `Продлить период` uses the existing update plus authoritative reload. Guest visibility and executable-rule guards remain server-owned and unchanged.

Recently closed bounded runtime block: **DANGEROUS ACTION AUDIT SLICE / PROMOTION CREATION AUDIT / DONE / MVP / STAGING-SMOKE-PASSED**. One `createPromotion` transaction inserts the parent, runs caller-connection `afterInsert`, rereads the parent and ordered rule projection, appends one safe actor-bearing `VENUE_PROMOTION_CREATED`, then commits once. Validation, denial, `afterInsert`, initial-rule, SQL or audit failure produces no success audit and rolls back all create-transaction writes. Configuration edit, schedule/target/reward, media and compatibility audits remain future.
...
the current schema and repository with no migration. Promotion configuration edit audit and broader
...
Open P2/future remains unchanged: promotion configuration edit audit, audit payload
