...
- `STAFF OPERATIONS SLICE B / OPTIONAL TEAM AND SCHEDULE MODULE / GUEST MANUAL OR SCHEDULE SOURCE /
  MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT`. Narrow settings RBAC,
  master-off retention/core access and MANUAL/SCHEDULE Guest privacy are locally validated; GitHub
  Actions, staging deploy and staging smoke are not claimed yet.
