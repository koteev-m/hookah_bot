...
| `staff_profile` | Public staff display profile and linked private venue-member relation. | Guest sees only opt-in public fields; linked user ids stay private; Owner keeps broad controls, while Manager manages only display-only/Staff-linked cards. |
...
| `staff_schedule` | Optional private venue planning over complete staff-shift intervals. | Owner/Manager manage only their venue; Staff sees own shifts plus safe overlapping colleagues; Guest/foreign/Platform-only access is denied. `STAFF SCHEDULE PHASE 1 / FUNCTIONALLY PASSED ON STAGING / PRODUCT AND RBAC POLISH REQUIRED`. |
...
| Venue Manager | `STAFF_ACCESS_VIEW`, `STAFF_INVITE_CREATE_STAFF`, `STAFF_INVITE_REVOKE_STAFF` | Own venue | Lists current members and pending Staff invites; creates/revokes Staff only. Manager/Owner/Admin targets are denied. |
| Venue Manager | `STAFF_PROFILE_MANAGE_STAFF`, `STAFF_PROFILE_PUBLISH_STAFF`, `STAFF_PROFILE_EDIT_OWN` | Own venue | Manages display-only/active-Staff-linked cards. Owner/other-Manager/missing/foreign linkage is protected; own Manager card permits safe self-edit only. |
...
| Venue Owner | `staff.manage`, `STAFF_ACCESS_VIEW`, `STAFF_INVITE_CREATE_STAFF`, `STAFF_INVITE_CREATE_MANAGER`, `STAFF_INVITE_REVOKE_STAFF`, `STAFF_INVITE_REVOKE_MANAGER`, `menu.manage`, `stop_list.manage`, `table_qr.manage/rotate/export`, `settings.manage`, `staff_chat.link/unlink/test` | Own venue | Keeps broad staff/profile controls; Owner/Admin invite targets remain outside venue flow. Dangerous actions need confirmation/audit. |
...
| Staff schedule | No Phase 1 flow. | No. | `STAFF SCHEDULE PHASE 1 / FUNCTIONALLY PASSED ON STAGING / PRODUCT AND RBAC POLISH REQUIRED`: Owner/Manager `График смен`; Staff read-only `Мои смены`. | No automatic authority. | No Phase 1 flow. | Venue Mini App is source of truth; effective hours are defaults only; Guest/Telegram/foreign access denied and Today Shift stays separate. |
...
| Staff profile created/updated/published/hidden | Transaction-bound `STAFF_PROFILE_*`; safe profile id, linkage class, changed field names and old/new visibility/linkage only. Denial/no-op/rollback has no success audit. |
...
| Staff profiles / today shift | `STAFF OPERATIONS SLICE A / MANAGER PARITY + SHIFT TIME DEFAULTS / MVP IMPLEMENTED / LOCAL VALIDATION PASSED`; canonical model is `docs/STAFF_PROFILES_SHIFTS_TIPS.md`. | Guest sees only public visible profile/shift data; Owner keeps broad controls; Manager manages display-only/Staff-linked cards; Staff may edit own safe linked fields only. | Keep protected-linkage, transaction-bound audit, privacy and Guest visibility in regression. |
| Staff Schedule Phase 1 | `STAFF SCHEDULE PHASE 1 / FUNCTIONALLY PASSED ON STAGING / PRODUCT AND RBAC POLISH REQUIRED`. Explicit runtime permissions/routes/UI reuse `staff_shifts` and `updated_at`; schedule-schema verdict `NO_MIGRATION_EXPECTED`. | Owner/Manager manage own-venue planned shifts; effective opening hours are create defaults only; Staff reads own plus safe overlapping colleagues; Guest/foreign/Platform-only access denied. | Slice A needs green Actions and new staging smoke; keep route denial/privacy, effective-hours, atomic audit and Today/Guest behavior in regression. No production-readiness claim yet. |
...
- Staff Schedule Phase 1:
  ...
  schedule-schema verdict `NO_MIGRATION_EXPECTED`; Slice A permission/privacy/default gates passed
  locally, while green Actions and a new staging smoke remain pending.
- Invite revoke rollout uses PostgreSQL V120/H2 V121: new binaries first, no old runtime instances
  ...
  selection remain Slice B future.
