...
- Guest cart stale menu recovery: **GUEST CART STALE MENU SELECTION RECOVERY / ITEM-LEVEL ACTION AND
  COPY POLISH / MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT**;
  preview/submit parity, expanded zero-write rejection, rollback and item-level removal/recalculation
  UX are local evidence. PostgreSQL `V123` and H2 `V124` add nullable `request_fingerprint` without
  backfill or global uniqueness.
