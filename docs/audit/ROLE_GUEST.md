...
- QR/table exit flow должен проверять user scoping: один guest выходит, другой guest за тем же столом остаётся в своём context.
...
8. Без active order/bill/staff call нажать `Завершить визит`; повторное открытие Mini App не должно restore table context, повторный QR scan должен re-enter.
