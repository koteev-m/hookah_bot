...
| Staff call | Bot table context can create staff call | Mini App staff call sends `tableSessionId`, backend notifies staff chat, and Guest Mini App can read simple lifecycle status | Done / M5 local smoke target | Staff chat runtime smoke still required per pilot venue | P2 | Keep smoke checklist step mandatory for each pilot venue. |
...
| Support | Bot chat remains support/fallback path; M4A Guest Bot booking replies persist to support threads | Guest Mini App M4B/M4C `Сообщения` shows thread cards with venue/context/status/last message/unread, active/resolved filters and resolve/reopen actions | Partial / M4B-M4C local | General tickets and platform support remain later; staging smoke must verify many threads across venues and conversation lifecycle separate from booking lifecycle | P1/P2 | Stage-smoke M4B/M4C inbox UX before broader support/ticket promises. |
...
| Staff calls lifecycle | Bot has active calls and callbacks for ack/done in staff chat router paths | Mini App has backend-backed `Вызовы` queue with accept/close and guest status labels | Done / staging smoke target | No cancel/SLA/escalation yet; staff chat group binding remains manual runtime dependency | P1 | Run M5 smoke: guest create/status, venue accept/close, staff chat notification and inline callbacks. |
