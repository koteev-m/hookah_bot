...
| Settings | Bot remains canonical for broad venue settings | Mini App settings route manages booking hold settings, paid shift extension settings and M8b structured public card basics (`countryCode`, `city`, `address`, `formattedAddress`, optional coordinates, `guestContact`, `cardDescription`) for OWNER/MANAGER; STAFF is hidden/forbidden | Partial / M8b code-e2e backed | Hours/exceptions, media/info sections, promotions, guest preview and billing remain bot/platform-canonical; Yandex geodata config and route behavior need staging smoke; no bulk settings endpoint | P1/P2 by slice | Continue small backend-backed settings slices and smoke M8b on staging before closure. |
...
| Feature | Bot | Mini App | Status | Gap | Priority | Recommendation |
|---|---|---|---|---|---|---|
| Venue profile/settings | Bot owner flows edit profile, description, hours, contacts and settings | Mini App has booking hold, paid shift extension and M8b structured public card basics with backend-proxied Yandex suggestions/manual fallback; venue name is read-only, and broader hours/media/promotions/preview settings remain bot-canonical | Partial / M8b code-e2e backed | Owner must still use bot/platform flows for broad venue setup beyond public location/contact/card description; live provider config is staging-pending | P1/P2 by slice | Implement real settings only as small backend-supported slices; smoke M8b on staging before marking closed. |
