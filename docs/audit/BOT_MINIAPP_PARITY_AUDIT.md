...
> Additional correction as of 2026-06-24: M7c adaptive transactional reminders are implemented locally but not staging-accepted. Guest and Venue Mini App booking cards expose `lastGuestConfirmationAt`; the Bot remains the background delivery channel through Telegram outbox. Runtime remains disabled by default and M7c must not be marked CLOSED until real Telegram smoke passes.
...
| Bookings | Bot booking flow exists, `/my` shows active bookings with public `Бронь №...` and `Держим до`; Bot sends M7c reminders only when the opt-in worker is explicitly enabled | Guest and Venue Mini App booking baselines exist; Venue M3 lifecycle and M7a hold-minutes settings are closed; Guest Mini App M7b `Мои брони` is implemented with `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`; attendance intent is shown when `lastGuestConfirmationAt` is present | Partial | M7c code is implemented but real Telegram staging smoke is pending; preorder remains later; M7b needs real Bot `/my` side-by-side and real two-account isolation smoke | P2 | Keep booking lifecycle, hold settings, M7c disabled-by-default behavior and Guest/Bot public booking label parity in regression. |
...
| Bookings | Bot has booking list/actions/settings and M7c transactional reminder callbacks (`Да, буду`, `Перенести`, `Отменить`) behind the disabled-by-default worker | Venue Mini App has booking queue/lifecycle actions, M7a hold-minutes settings and guest-confirmed visit indicator; Guest Mini App has account-level `Мои брони` with change/cancel actions and attendance indicator | Done / smoke for lifecycle and M7a; M7b `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`; M7c code/test-backed, staging smoke pending | M7c must pass real Telegram smoke before CLOSED; preorder remains later | P2 | Keep booking lifecycle, hold settings, RBAC split, public booking label parity, reminder feature flag and outbox `QUEUED` semantics in regression. |
