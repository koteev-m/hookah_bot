...
| Option stop-list | Bot stop-list supports item/options and bot-only base flavor profile helpers | Mini App displays item-scoped options/flavors, structured guest selected options and explicit item/option stop-list toggles | Mostly aligned | Mini App still lacks bot's `Добавить базовые вкусы` / normalize helper actions; these need a shared item-scoped template/batch API before UI parity | P2 | Keep manual `Добавить вкус` now; add shared base flavor template API later without global flavors. |
