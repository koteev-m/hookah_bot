...
> Additional correction as of 2026-06-18: M4B/M4C support inbox cards, unread/read receipts and resolve/reopen lifecycle are code/test-backed; staging multi-venue runtime smoke remains required. M5 staff-call lifecycle and compact Guest Mini App staff-call UX are code/e2e-backed. M6 staff-chat diagnostics/unlink polish is code/e2e-backed: Mini App now exposes status/link/test/unlink semantics with owner-only unlink and masked chat id. Manual Telegram staff-chat notification/link/test/unlink smoke remains required. Venue Mini App order/bill controls are no longer bot-only for manager/owner. Treat older matrix rows below as historical where they conflict with the canonical roadmap and smoke checklist.
...
- `M4B/M4C support inbox not implemented` is outdated in code/tests; staging multi-venue runtime smoke is still required.
- `M5 staff calls lifecycle missing` is outdated in code/e2e; manual Telegram group notification smoke is still required.
...
| Staff call | Bot table context can create staff call and staff chat callbacks can ACK/DONE | Mini App staff call sends `tableSessionId`, backend notifies staff chat, Guest Mini App uses compact NEW/ACK/DONE status, and Venue Mini App `Вызовы` can accept/close | Done / code+e2e-backed | Staff chat runtime smoke still required per pilot venue | P2 | Keep smoke checklist step mandatory for each pilot venue. |
...
| Bookings | Bot booking flow exists | Guest and Venue Mini App booking baselines exist; Venue M3 lifecycle is implemented | Partial | Reminders/settings/preorder remain later | P2 | Keep booking lifecycle in regression; do not expose reminders/settings without backend-backed UI. |
| Support | Bot chat remains support/fallback path; M4A Guest Bot booking replies persist to support threads | Guest Mini App M4B/M4C `Сообщения` shows thread cards with venue/context/status/last message/unread, active/resolved filters and resolve/reopen actions | Partial / code+test-backed | General tickets and platform support remain later; staging smoke must verify many threads across venues and conversation lifecycle separate from booking lifecycle | P1/P2 | Stage-smoke M4B/M4C inbox UX before broader support/ticket promises. |
...
| Staff calls lifecycle | Bot has active calls and callbacks for ack/done in staff chat router paths | Mini App has backend-backed `Вызовы` queue with accept/close and compact guest status labels | Done / code+e2e-backed | No cancel/SLA/escalation yet; staff chat group binding remains manual runtime dependency | P1 | Run manual M5 smoke for linked Telegram group notification and inline callbacks. |
...
| Staff chat link | Bot supports staff chat bind/status/test/unlink | Mini App chat link/status screen shows linked/unlinked state, masked chat id, backend-built link command, outbox-backed test result and OWNER-only unlink confirmation | Done / code+e2e-backed | Real Telegram group link/test/unlink and operational notification delivery still need manual runtime smoke per pilot venue | P1 | Keep M6 in regression and smoke linked group behavior before staging closure. |
| Settings | Bot remains canonical for broad venue settings | Mini App settings route currently manages paid shift extension settings; broader profile/hours/booking settings remain bot-canonical | Partial | Generic settings parity is not done; avoid bulk settings endpoint | P1 if settings required in Mini App | Implement small backend-backed settings slices later. |
| Bookings | Bot has booking list/actions | Venue Mini App has booking queue/lifecycle actions; Guest booking baseline exists | Done / smoke | Reminders/settings/preorder remain later | P2 | Keep booking lifecycle and RBAC split in regression. |
...
| Feature | Bot | Mini App | Status | Gap | Priority | Recommendation |
|---|---|---|---|---|---|---|
| Venue profile/settings | Bot owner flows edit profile, description, hours, contacts and settings | Mini App has settings only for paid shift extension; broader profile/hours/booking settings remain bot-canonical | Partial | Owner must use bot for broad venue setup | P1 | Implement real settings only as small backend-supported slices. |
