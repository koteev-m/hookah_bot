...
| Bookings | Bot booking flow exists, `/my` shows active bookings with public `Бронь №...` and `Держим до` | Guest and Venue Mini App booking baselines exist; Venue M3 lifecycle and M7a hold-minutes settings are closed; Guest Mini App M7b `Мои брони` is implemented for staging smoke | Partial | Runtime reminders/preorder remain later; M7b needs staging smoke | P2 | Keep booking lifecycle, hold settings and Guest/Bot public booking label parity in regression; do not expose reminders without backend-backed runtime. |
...
| Bookings | Bot has booking list/actions/settings | Venue Mini App has booking queue/lifecycle actions and M7a hold-minutes settings; Guest Mini App has account-level `Мои брони` with change/cancel actions | Done / smoke for lifecycle and M7a; M7b staging target | Runtime reminders/preorder remain later | P2 | Keep booking lifecycle, hold settings, RBAC split and public booking label parity in regression. |
