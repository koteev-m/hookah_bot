...
| Bookings | Bot booking flow exists | Guest and Venue Mini App booking baselines exist; Venue M3 lifecycle and M7a hold-minutes settings are implemented | Partial | Reminders/preorder remain later; M7a needs staging smoke | P2 | Keep booking lifecycle and hold settings in regression; do not expose reminders without backend-backed UI. |
...
| Bookings | Bot has booking list/actions/settings | Venue Mini App has booking queue/lifecycle actions and M7a hold-minutes settings; Guest booking baseline exists | Done / smoke for lifecycle, M7a staging target | Reminders/preorder remain later | P2 | Keep booking lifecycle, hold settings and RBAC split in regression. |
