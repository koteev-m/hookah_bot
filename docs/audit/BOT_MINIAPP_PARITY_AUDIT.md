...
> Additional correction as of 2026-06-18: M4B/M4C support inbox cards, unread/read receipts and resolve/reopen lifecycle are code/test-backed; staging multi-venue runtime smoke remains required. M5 staff-call lifecycle and compact Guest Mini App staff-call UX are code/e2e-backed; manual Telegram staff-chat notification smoke remains required. Venue Mini App order/bill controls are no longer bot-only for manager/owner. Treat older matrix rows below as historical where they conflict with the canonical roadmap and smoke checklist.
...
| Staff chat link | Bot supports staff chat bind/status/test/unlink | Mini App chat link/status screen exists; backend/wrapper support owner-only unlink but UI does not expose it yet | Partial | Needs Mini App owner-only unlink/diagnostics polish and runtime verification in Telegram group | P1 | Next bounded milestone: staff chat diagnostics/unlink polish. |
