...
> Additional correction as of post-M9a checkpoint on 2026-06-25: M8a/M8b-Free public profile/card settings is CLOSED / staging smoke passed in provider-free mode, with Yandex adapters still optional disabled integrations. M9a Deployment SSH Reliability Hardening is CLOSED / staging smoke passed; the ControlMaster helper is committed and validated as an opt-in resilient deploy path, while the exact fresh SSH failure cause remains unconfirmed. The selected next parity milestone is M9b Venue Working Hours and Date Exceptions Mini App Parity: backend/Bot support schedule and date overrides, but Venue Mini App lacks equivalent settings/open-state read models and direct Mini App booking requests currently are not validated against venue hours.
> Additional correction as of M9b.2 local checkpoint: M9b Venue Working Hours and Date Exceptions Mini App Parity plus M9b.1 inclusive exception ranges/rejection copy is implemented locally, and M9b.2 addresses the staging-discovered save/list UX issue for changed-hours exceptions. M9b is not closed until staging smoke proves owner/manager schedule settings, changed-hours row visibility, guest open-state, booking validation, STAFF denial and Bot closed-date parity.
...
- venue hours/date exceptions are implemented locally through M9b/M9b.1/M9b.2, but staging smoke remains required before closing the parity slice;
...
| Settings | Bot remains canonical for broad venue settings | Mini App settings route manages booking hold settings, paid shift extension settings, M8a/M8b-Free structured public card basics (`countryCode`, `city`, `address`, `formattedAddress`, optional coordinates, `guestContact`, `cardDescription`) and M9b weekly hours/date exceptions for OWNER/MANAGER; STAFF is hidden/forbidden | Partial / M8a-M8b-Free staging-smoke closed; M9b local validation pending staging smoke | Media/info sections, promotions, guest preview and billing remain bot/platform-canonical; local city seed is intentionally incomplete and manual input remains fallback; no bulk settings endpoint | P1/P2 by slice | Stage-smoke M9b before closure; continue small backend-backed settings slices and keep provider-free public card/location in regression. |
...
| Feature | Bot | Mini App | Status | Gap | Priority | Recommendation |
|---|---|---|---|---|---|---|
| Venue profile/settings | Bot owner flows edit profile, description, hours, contacts and settings | Mini App has booking hold, paid shift extension, M8a/M8b-Free structured public card basics with local country/city suggestions/manual address, and M9b weekly hours/date exceptions; venue name is read-only, and broader media/promotions/preview settings remain bot-canonical | Partial / M8a-M8b-Free staging-smoke closed; M9b local validation pending staging smoke | Owner must still use bot/platform flows for broad setup beyond public location/contact/card description and schedule; optional Yandex adapters are disabled/commercial-only and not required for production-default flow | P1/P2 by slice | Stage-smoke M9b before closure; implement further real settings only as small backend-supported slices. |
