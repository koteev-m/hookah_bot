...
>
> Current correction as of 2026-07-30: the current contract supersedes the old Published/Draft
> split. One `Предпросмотр для гостя` calls `GET /api/venue/{venueId}/guest-preview`; the backend
> selects `PUBLISHED_PUBLIC` or `PRIVATE_DRAFT` through the shared public-facing assembly.
> Published uses unchanged public Guest guards, Private Draft exposes only saved public-facing
> state to own-venue OWNER/MANAGER, and the public Guest API cannot reach draft. Dirty settings
...
- guest profile edit/promotions/loyalty progress screens не доведены до bot parity; History and venue-only Favorites are staging-smoke-passed;
...
| Favorites | Bot favorite venues/items flows exist; Catalog and Profile open the same venue list with source-aware Back | Mini App catalog/detail add/remove and Account venue-only list/actions use the shared backend source | DONE / MVP / STAGING-SMOKE-PASSED for favorite venues | Favorite menu items/options remain future and are not shown in Phase 1 | Regression | Keep current-user isolation, unavailable filtering/restoration and Bot ↔ Mini App synchronization in smoke. |
...
| Settings | Bot remains canonical for broad venue settings and media authoring | Mini App settings route manages booking hold settings, paid shift extension settings, M8a/M8b-Free structured public card basics (`countryCode`, `city`, `address`, `formattedAddress`, optional coordinates, `guestContact`, `cardDescription`) and M9b weekly hours/date exceptions for OWNER/MANAGER; one read-only `Предпросмотр для гостя` uses backend-selected `PUBLISHED_PUBLIC` / `PRIVATE_DRAFT`; STAFF is hidden/forbidden | Partial overall; M8a-M8b-Free, M9b-M9b.3 and unified Guest Preview are staging-smoke closed | Media/info-section authoring and broader settings remain Bot-first; local city seed is intentionally incomplete and manual input remains fallback; no bulk settings endpoint | P1/P2 by slice | Keep provider-free public card/location, schedule validation and unified preview RBAC/privacy/saved-state behavior in regression; continue only bounded backend-backed settings slices. |
...
| Feature | Bot | Mini App | Status | Gap | Priority | Recommendation |
|---|---|---|---|---|---|---|
| Venue profile/settings | Bot owner flows edit profile, description, hours, contacts and settings | Mini App has booking hold, paid shift extension, M8a/M8b-Free structured public card basics with local country/city suggestions/manual address, M9b weekly hours/date exceptions and unified saved-state Guest Preview; venue name is read-only, and broader media authoring remains bot-canonical | Partial overall; M8a-M8b-Free, M9b-M9b.3 and unified Guest Preview staging-smoke closed | Owner must still use bot/platform flows for broad setup beyond public location/contact/card description, schedule and read-only preview; optional Yandex adapters are disabled/commercial-only and not required for production-default flow | P1/P2 by slice | Implement further real settings only as small backend-supported slices; keep M9b schedule and preview RBAC/privacy/dirty-state behavior in regression. |
...
|---|---|---|---|---|---|---|
| GUEST | Full guest menu/table context; Catalog and Profile expose favorite venues | Guest Mini App has catalog/table/menu/cart/order/support plus Account History, Feedback, bookings and venue-only Favorites | Partial beyond closed slices | Profile edit, promotions and loyalty progress remain partial; venue favorites are closed | P1/P2 | Keep History/Feedback/Favorites in regression; scope remaining account/promotions separately. |
...
- Still active from this section: Guest profile edit/promotions/loyalty parity, Platform cockpit placements/analytics partial sections, frontend/browser coverage beyond the current smoke harness and broad test heap/runtime risk. Venue-only Favorites are no longer an active gap.
