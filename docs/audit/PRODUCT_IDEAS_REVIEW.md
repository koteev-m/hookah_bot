...
- P1 ops: table-context exit, guest/staff bill display parity, bill request/payment method UX, staff-chat activity card, manual platform billing cockpit, renewal/courtesy, staff invite sharing and support/tickets MVP are now staging/smoke-closed; provider payment decision is the next launch-commercial gap before broad growth.
...
4. Unified staff notifications beyond the closed core if personal subscriptions/history are needed.
5. Owner description section defaults `Схема зала` and `Интерьер`; expose sections to guests.
