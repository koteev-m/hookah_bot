...
- P1 ops: table-context exit, guest/staff bill display parity, bill request/payment method UX, staff-chat activity card, manual platform billing cockpit, renewal/courtesy and staff invite sharing are now staging-closed; support/tickets and provider payment decisions are the next launch-commercial/operations gaps before broad growth.
...
Делать сейчас:
- Support/tickets MVP beyond booking threads unless pilot commercial payment becomes the stronger blocker.
- Keep manual platform billing cockpit, renewal/courtesy, staff invite sharing, Mini App mutation, table-context exit, booking lifecycle, bill/bill-request parity, staff-chat activity card, notification reliability and multi-venue Telegram selector in regression smoke.
...
1. Support/tickets MVP beyond booking threads if pilot operations need general support routing.
2. Real acquiring provider / Telegram Stars provider decision if pilot commercial payment requires online payment.
3. Booking lifecycle, reminder rollout, table-context exit, bill/bill-request parity, staff-chat activity card, manual billing renewal/courtesy and multi-venue selector as regression smoke unless new gaps appear.
