...
| B. Settings / venue profile-card and schedule | Bot implements profile/card fields, address, contacts, description/info sections, weekly hours and date-specific open/closed overrides. | Implemented for M8a/M8b-Free and M9b/M9b.1/M9b.2/M9b.3: public-card route covers structured location/contact/description; optional Yandex geodata proxy is config-gated, disabled by default and configured with separate Geosuggest/Geocoder keys; guest venue read models expose display address, existing coordinate routes and text-route fallback; schedule settings routes reuse existing booking-hours/date-override tables; date-exception ranges expand to per-date rows with nullable guest-facing `guest_note`; guest read models expose today's safe schedule/open state; direct Guest Mini App booking create/update validates against configured hours and returns human schedule rejection codes/copy. | Implemented / staging smoke passed: current `settings` screen manages booking hold, paid shift extension, structured public card basics, weekly hours and compact date-exception ranges with optional reason/comment, with weekly base schedule separated from concrete-date overrides. Saved exceptions close/reset the form and refresh the compact list; closed and changed-hours grouped exceptions can be edited to new inclusive date ranges. Guest catalog/card shows today schedule/open state. | OWNER/MANAGER only for settings writes; STAFF hidden/forbidden. Guest schedule/open-state reads preserve published/subscription visibility gates. Guest-facing reason/comment is explicit public copy; private admin notes must not leak. | `VenueBookingHoursRepository.kt`, `VenueRoutes.kt`, `VenueRbac.kt`, `GuestVenueRoutes.kt`, `GuestBookingRoutes.kt`, `TelegramBotRouter.kt`, `venueSettings.ts`, `guestVenue.ts`, `catalog.ts`, `venueApi.ts`, `guestDtos.ts` | Medium: timezone and overnight hours semantics must stay aligned with Bot; keep schedule validation in regression. | M9b/M9b.1/M9b.2/M9b.3 CLOSED / staging smoke passed. Keep M8b-Free provider-free public card fields and Bot booking slot behavior in regression. | Backend route/RBAC tests, Guest booking validation tests, Mini App build/e2e and owner staging schedule smoke passed; keep owner/manager schedule, range exception/reason, guest booking/open-state, STAFF denial and Bot closed-date paths in regression. |
| B. Paid shift extension settings | Implemented in Mini App and backend; guest-facing extension visibility gating is CLOSED / staging smoke passed; bot ordering entry exists, while Owner/Manager Bot settings parity remains a separate follow-up if needed. | Implemented: settings and pending request routes. | Implemented: settings screen; pending requests screen exists; guest entry appears only when active order/table state makes extension actionable. | `SHIFT_EXTENSION_VIEW`, `SHIFT_EXTENSION_CONFIRM`, `SHIFT_EXTENSION_SETTINGS`; STAFF has view/confirm but no settings. | `ShiftExtensionRoutes.kt`, `venueSettings.ts`, `venueShiftExtensions.ts`, `venueApp.ts`, `guestApp.ts` | Low for current guest gating; Medium for optional bot settings parity. | Guest table context cleanup CLOSED; defer Owner/Manager Bot settings parity behind guest/staff bill correctness unless a pilot venue requires it. | Mini App build/e2e; shift extension route tests if changed; regression smoke for hidden no-order state and active-order visibility. |
...
| H. Venue card preview | Implemented in bot: publish readiness and guest preview callbacks. | One `GET /api/venue/{venueId}/guest-preview` selects `PUBLISHED_PUBLIC` through unchanged Guest guards or `PRIVATE_DRAFT` through the shared public-facing assembly; the public Guest API cannot select or reach draft. | **DONE / MVP / STAGING-SMOKE-PASSED**: one `Предпросмотр для гостя` screen with global/contextual entries; Published shows exact current Guest state, Private Draft shows saved public-facing state, both are read-only. | OWNER/MANAGER own venue only; STAFF/foreign denied; Platform scope does not auto-grant access; ARCHIVED/DELETED/missing fail closed. | `GuestVenueRoutes.kt`, `GuestVenueReadService.kt`, `VenueGuestPreviewRoutes.kt`, `VenueGuestPreviewDtos.kt`, `venueGuestPreview.ts`, `venueApp.ts`, `venueSettings.ts` | Keep lifecycle, hidden/private fields, scoped media, dirty saved-state and stale-response isolation in regression. | Unified Published + Private Draft: **DONE / MVP / STAGING-SMOKE-PASSED**. | Focused route/RBAC/privacy coverage plus green Actions and staging smoke for parity, roles, lifecycle modes, both origins, scoped media and stale switching. |
| I. Venue/public-card media management | Bot OWNER/MANAGER add image/PDF attachments to info sections, delete an attachment and hide/show the whole section; storage is Telegram `file_id`. | Existing Guest media proxy and read-only Preview delivery only; `PRIVATE_DRAFT` uses an authenticated venue/section/media-scoped proxy; no Venue upload/author/manage endpoint. | Venue Mini App has no file picker or media management UI. Both preview modes display only visible saved media through their guarded/scoped routes and expose no raw refs. | OWNER/MANAGER for own venue; Guest receives guest-safe proxy URL only. | `VenueInfoSectionsRepository.kt`, `VenueInfoSectionMediaRepository.kt`, `TelegramBotRouter.kt`, `GuestVenueRoutes.kt`, `GuestVenueReadService.kt`, `VenueGuestPreviewRoutes.kt`, `guestVenue.ts` | High until storage/bridge, validation, deletion and audit semantics are specified. | Public-card media: **PARTIAL / BOT-FIRST**. Venue Mini App upload: **MISSING / FUTURE**. | **NEEDS_MEDIA_STORAGE_SPEC_FIRST** before a runtime slice. |
...
- Historical M1 gate: do not show a feature until real screens/routes exist. `Статистика` later satisfied this gate after M2; unified `Предпросмотр для гостя` now satisfies it for backend-selected Published/Public and saved Private Draft modes.
...
- Do not build full Platform Support Center, generic CRM, general tickets or structured reschedule proposals in this M4A slice; the later Guest Communication UX / Support Tickets MVP superseded the general-ticket and Platform support-ticket gap.
...
- Historical M4A-only gap superseded by the later Guest Communication UX / Support Tickets MVP: general support tickets and Platform support-ticket center are now smoke-passed. Advanced support automation, diagnostics, attachments, macros, CSAT and analytics remain follow-ups.
...
Status: CLOSED / staging smoke passed. Historical M4B scope did not include Platform Support Center; the later Support Tickets MVP provides the backend-backed support-ticket center.
...
- Platform support-ticket visibility is now handled by the later backend-backed Support Tickets MVP; no fake Platform Support Center entries should be added outside those routes/screens.
...
- Historical M4B-only deferral for guest venue questions is superseded by the later Guest Communication UX slice: catalog/venue detail `Задать вопрос` opens or reuses `VENUE_CHAT`.
- Structured `Предложить другое время`, support diagnostics, SLA/escalation, macros, CSAT, attachments and search remain follow-ups.
...
- Platform support-ticket queues are exposed only through the later backend-backed Support Tickets MVP routes and permissions; ordinary `VENUE_CHAT` remains hidden from Platform.

### M4C — Support Thread Resolve/Reopen Lifecycle

Status: CLOSED / staging smoke passed. Historical M4C scope did not include Platform Support Center; the later Support Tickets MVP provides the backend-backed support-ticket center.
...
- Same `fromDate`/`toDate` means a one-day exception.
- Saving a new or edited range upserts each target date; overlapping target dates are deterministically overwritten by the latest saved closed/special-hours values and optional guest-facing reason/comment.
...
- M9b.3 correction: editing an existing closed or changed-hours grouped exception allows changing the date range; backend replacement deletes the original grouped dates and upserts the new inclusive range atomically.
...
- OWNER/MANAGER can edit an existing exception's start/end date, reason/comment and special hours without leaving stale old rows.
...
- M9b.1/M9b.2/M9b.3 validation covers focused backend schedule, guest booking, Telegram closed-date and Mini App exception UX tests; broad Telegram wildcard may still need splitting if local JVM stalls.
- Staging smoke passed after M9b.3: OWNER schedule/date-exception smoke confirmed closed and changed-hours periods can be created, edited, have their date ranges changed and deleted; old stale rows disappear; guest booking rejects closed/out-of-hours dates with human copy; Bot closed-date selection shows human copy plus `📅 К выбору дат` and `🏠 В каталог`.
...
### Implemented — Venue Card Preview

Scope:

- Unified Guest Preview is **VENUE MINI APP GUEST PREVIEW / PUBLISHED + PRIVATE DRAFT READ-ONLY /
  DONE / MVP / STAGING-SMOKE-PASSED**.
- One global and one public-card-settings entry open the same read-only `Предпросмотр для гостя`
  renderer and call one `GET /api/venue/{venueId}/guest-preview`.
- Backend selects `PUBLISHED_PUBLIC` only through unchanged Guest lifecycle/subscription guards or
  `PRIVATE_DRAFT` through the shared public-facing assembly of saved state; the public Guest API
  cannot select or reach draft.
- Both modes preserve child visibility. Private media uses only the authenticated
  venue/section/media-scoped preview proxy; hidden media and raw refs remain absent.
- DRAFT/HIDDEN/PAUSED/SUSPENDED or guest-blocked venues can use authenticated private preview;
  ARCHIVED/DELETED/missing fail safely. Unsaved form state blocks navigation and is never
  auto-saved; versioned snapshots, publish actions, mutations and public/share links remain out of
  scope.

Definition of Done:

- `PUBLISHED_PUBLIC` matches Guest Mini App visibility rules exactly.
- `PRIVATE_DRAFT` preserves sub-entity visibility and exposes no private
  staff/order/billing/provider/audit data.
- STAFF/foreign access is denied; venue switching clears and ignores stale responses.
- Local focused coverage, green GitHub Actions, staging deploy and staging role/parity/privacy/stale-state smoke pass.

### Future — Venue Mini App Media Upload & Management Foundation

Current capability matrix:

| Surface | Bot current | Venue Mini App current | Guest display | Target |
| --- | --- | --- | --- | --- |
| Public card section image/PDF | Add multiple, delete attachment, hide/show whole section; Telegram `file_id`; no direct replace/per-file hide. | View-only through both Preview modes; no author/upload/manage flow. | Guest Bot sends Telegram media; Guest Mini App/`PUBLISHED_PUBLIC` use guarded Guest proxy URLs; `PRIVATE_DRAFT` uses the authenticated scoped preview proxy. | Safe shared source/bridge, validate, replace, visibility, delete, audit, guest-safe URL. |
| Structured menu item photo/description/thumbnail | No current photo/thumbnail CRUD; current repository does not wire the legacy description column. | No photo/description/thumbnail or option/flavor media contract. | Not rendered in current structured Guest menu. | Item-safe media/description after storage/proxy spec. |
| Photo/PDF menu | Flat `section_type=menu` info section; add/delete and whole-section visibility. | No management; both Preview modes are view-only through guarded/scoped delivery. | View-only pre-QR info; never orderable by itself. | Manage through the same foundation without merging it with structured menu. |
| Staff profile photo | No supported upload/manage flow. | `Фото сотрудника — позже`; raw manual input hidden. | Initials placeholder; both Preview modes omit raw photo ref. | Separate employee-consent, moderation, replacement/deletion and approved delivery rules. |

Minimum future scope:

- OWNER/MANAGER file picker; image upload; PDF only for allowed media surfaces;
- server-side MIME/type/size validation plus venue ownership/scoping;
- safe storage abstraction; replacement; hide/show; deletion; guest-safe delivery URL;
- audit upload/replace/delete;
- no raw Telegram `file_id` or storage key in public DTOs;
- one media source of truth or a compatible Bot/Mini App bridge;
- staff-photo consent remains a separate mandatory rule.

Verdict: **NEEDS_MEDIA_STORAGE_SPEC_FIRST**. Current public-card media is coupled to Telegram
`file_id`; browser upload, safe public delivery, deletion semantics and cross-source migration/bridge
are not specified. Do not choose an object-storage product or start the cross-layer runtime diff in
this parity docs sync.
