...
| B. Settings / venue profile-card | Implemented in bot: profile/card fields, address, contacts, description/info sections, hours. | Partial: public-card route covers structured location/contact/description; optional Yandex geodata proxy is config-gated and disabled by default; guest venue read models expose display address, existing coordinate routes and text-route fallback; no broad settings API for hours/media. | Partial / M8b-Free code-e2e backed: current `settings` screen manages booking hold, paid shift extension and structured public card basics with dirty/save/focus UX, local country/city suggestions and manual address entry; broader profile/hours/media settings remain bot-canonical. | OWNER/MANAGER only for public card/location provider routes; STAFF hidden/forbidden. | `VenueRepository.kt`, `VenueRoutes.kt`, `GuestVenueRoutes.kt`, `TelegramBotRouter.kt`, `venueSettings.ts`, `guestVenue.ts` | Medium: provider-free route destination needs staging smoke; full local street/house suggestions require a separate ФИАС/ГАР import design. | Keep M8b-Free in staging regression; continue future settings only as small backend-supported slices. | Route/RBAC/provider tests, Mini App e2e, provider-free manual location smoke. |
