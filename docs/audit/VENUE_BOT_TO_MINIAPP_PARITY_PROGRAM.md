...
- Published Guest Preview Phase 1 is **MVP IMPLEMENTED / LOCAL VALIDATION PASSED**: OWNER/MANAGER use the exact Guest-visible read model and unchanged lifecycle/subscription guards.
- Draft Preview Phase 2.1 is **MVP IMPLEMENTED / LOCAL VALIDATION PASSED**: a separate OWNER/MANAGER-only Venue route/read model exposes saved `DRAFT` public-candidate data without a Guest API bypass.
...
- HIDDEN/PAUSED/SUSPENDED/ARCHIVED/DELETED preview, unsaved form state, versioned snapshots, publish actions, mutations and public/share links remain out of scope. CI and staging smoke remain pending.
...
- Local focused backend selectors, promotion repository regression, backend lint/compile, Mini App build and full deterministic e2e `92/92` pass.
