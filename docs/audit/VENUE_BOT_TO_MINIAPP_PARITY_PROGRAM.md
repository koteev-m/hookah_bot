...
Telegram Bot остаётся самой широкой venue-management поверхностью: selected venue hub уже разделён на `Работа смены`, `Настройка заведения`, `Статистика`, `Продвижение`, `Предпросмотр для гостя`. Venue Mini App уже покрывает большую часть operational core, но навигация пока плоская и часть реализованных экранов плохо обнаруживается. Главное правило программы: Mini App показывает только работающие backend-backed экраны; `Статистика`, `Продвижение` и `Предпросмотр для гостя` не должны появляться как основные кнопки до появления реальных Mini App routes/screens.

Первый безопасный milestone: привести Venue Mini App shell к bot-like information architecture и сделать видимым уже реализованный экран `Продления` для ролей с `SHIFT_EXTENSION_VIEW`. Это frontend-only изменение в `miniapp/src/screens/venueApp.ts` + CSS, без новых API, миграций или бизнес-логики.
...
| F. Statistics | Implemented per venue in bot; aggregate `owner_stats_all` is placeholder/limited. | Partial: `VenueStatsRepository` exists, but no Mini App venue stats route found. | Missing: no venue stats screen. | Need read permission decision before Mini App route. | `VenueStatsRepository.kt`, `TelegramBotRouter.kt` stats handlers | Medium/High: SQL coverage direct tests are thin; route/API missing. | M2 read-only stats API + screen after M1. | Repository SQL tests + route RBAC tests + Mini App build. |
...
- Stats gap: bot has stats, but Mini App lacks route/screen; adding UI before backend tests would be risky.
