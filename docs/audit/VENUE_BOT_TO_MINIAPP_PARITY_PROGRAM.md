...
- M7a first slice: booking hold settings in Venue Mini App. Reuse existing bot/backend hold-minutes semantics; show current hold duration and allow OWNER/MANAGER to update it. STAFF stays denied/hidden.
...
- Settings mismatch: `VENUE_SETTINGS` can imply a broad settings screen while current Mini App settings only controls shift extension settings. M7a should start with booking hold settings, not a bulk settings rewrite.
