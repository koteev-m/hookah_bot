...
| A. Work shift / bookings | Implemented/partial. Bot has booking list, confirm/cancel/change/message, seated/no-show, hold settings. | Implemented: booking list/status/message routes, reminder/expiry workers, and booking hold settings route using `venue_booking_settings.hold_minutes`. | Implemented: booking list, confirm/cancel/change, `Написать гостю`, seated/no-show, venue-local display, hold deadline display, and M7a `Настройки брони` hold-minutes control. | `BOOKING_VIEW`; `BOOKING_ARRIVAL_UPDATE`; `BOOKING_MANAGE` for management/message/settings actions. STAFF view/arrival only; settings stay OWNER/MANAGER. | `VenueBookingRoutes.kt`, `GuestBookingRepository.kt`, `venueBookings.ts`, `venueSettings.ts`, `TelegramBotRouter.kt` | Medium: reminders/preorder remain out of Mini App; M7a needs staging smoke. | M3 closed; M7a implemented / staging smoke target. | Booking route/RBAC tests, Mini App booking/settings e2e, Telegram runtime smoke. |
...
- M7a first slice implemented / staging smoke target: booking hold settings in Venue Mini App reuse existing bot/backend hold-minutes semantics; OWNER/MANAGER can update, STAFF stays denied/hidden.
