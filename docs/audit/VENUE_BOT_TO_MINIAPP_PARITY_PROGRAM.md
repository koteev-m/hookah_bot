...
| A. Work shift / bookings | Implemented/partial. Bot has `/my` active booking list/actions, confirm/cancel/change/message, seated/no-show, hold settings. | Implemented: booking list/status/message routes, expiry foundation, legacy reminder repository/worker gated off by default, booking hold settings route using `venue_booking_settings.hold_minutes`, and Guest account-level active booking route. | Implemented: Venue booking list/actions, `Написать гостю`, seated/no-show, venue-local display, hold deadline display, M7a `Настройки брони`, and M7b Guest Mini App `Мои брони`. | `BOOKING_VIEW`; `BOOKING_ARRIVAL_UPDATE`; `BOOKING_MANAGE` for venue management/message/settings actions. Guest list is user-scoped. STAFF view/arrival only; settings stay OWNER/MANAGER. | `GuestBookingRoutes.kt`, `GuestBookingRepository.kt`, `VenueBookingRoutes.kt`, `guestBookings.ts`, `venueBookings.ts`, `venueSettings.ts`, `TelegramBotRouter.kt`, `BookingReminderWorkerConfig.kt` | Medium: accepted adaptive M7c reminders and preorder remain later; keep public display number parity in regression. | M3 closed; M7a CLOSED; M7b implemented / `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`; M7c safety gate implemented; adaptive reminders next. | Booking route/RBAC tests, Mini App booking/settings e2e, Telegram `/my` runtime smoke, reminder feature-flag tests. |
...
| B. Settings / venue profile-card | Implemented in bot: profile/card fields, address, contacts, description/info sections, hours. | Partial: guest venue/info section routes exist; no broad Venue Mini App settings API for all fields. Booking hold settings are implemented as a focused M7a slice. | Partial: current `settings` screen manages booking hold settings and paid shift extension settings; broader profile/hours/card settings remain bot-canonical. | `VENUE_SETTINGS` exists but has no matching broad route; `BOOKING_MANAGE` covers M7a; `SHIFT_EXTENSION_SETTINGS` works. | `VenueRepository.kt`, `VenueInfoSectionsRepository.kt`, `VenueBookingHoursRepository.kt`, `VenueSettingsRepository.kt`, `venueSettings.ts` | High: exposing generic settings would create dead ends or partial writes. | Continue M7 small settings APIs by field; keep bot canonical for broader setup meanwhile. | Route/RBAC tests per setting slice. |
...
- M7b implemented with local validation and staging audit verdict `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`: Guest Mini App `Профиль → Мои брони` lists active/upcoming bookings across venues with bot-compatible public labels, venue-local time/deadline fields and guest change/cancel actions. Remaining runtime checks: real Bot `/my` side-by-side and real two-account isolation.
- M7c safety gate implemented: legacy booking reminder runtime is disabled by default and starts only with explicit opt-in. Existing legacy reminder rows are not deleted automatically.
- M7c adaptive reminders remain next: one reminder max per confirmed/changed booking in MVP, final adaptive policy, data reconciliation and Telegram staging smoke required before enabling the flag.
...
- Guest booking parity is not marked fully closed until both Bot `/my` and Guest Mini App `Мои брони` are compared against the same real booking and real cross-account isolation is smoke-tested.
...
- Settings mismatch: `VENUE_SETTINGS` can imply a broad settings screen while current Mini App settings only controls booking hold and shift extension settings. Continue with small settings slices, not a bulk settings rewrite.
