...
| H. Venue card preview | Implemented in bot: publish readiness and guest preview callbacks. | Published Preview uses the unchanged Guest read assembly/availability guards; Draft Phase 2.1 uses separate `GET /api/venue/{venueId}/draft-preview` and an allowlisted Venue read model. | **DONE / MVP / STAGING-SMOKE-PASSED**: one `Предпросмотр карточки` screen with global/contextual entries; Published shows exact current Guest state, Draft shows saved candidate state, both are read-only. | OWNER/MANAGER own venue only; STAFF/foreign denied; Platform scope does not auto-grant access; draft endpoint is `DRAFT`-only. | `GuestVenueRoutes.kt`, `VenueGuestPreviewRoutes.kt`, `VenueDraftPreviewRoutes.kt`, `VenueDraftPreviewReadService.kt`, `venueGuestPreview.ts`, `venueApp.ts`, `venueSettings.ts` | Keep lifecycle, hidden/private fields, draft media refs and stale-state isolation in regression. | Published Phase 1 + Draft Phase 2.1: **DONE / MVP / STAGING-SMOKE-PASSED**. | Focused route/RBAC/privacy coverage plus green Actions and staging smoke for parity, roles, statuses, both entrypoints, media hint and stale switching. |
| I. Venue/public-card media management | Bot OWNER/MANAGER add image/PDF attachments to info sections, delete an attachment and hide/show the whole section; storage is Telegram `file_id`. | Existing Guest media proxy and Preview reads only; no Venue upload/author/manage endpoint. | Venue Mini App has no file picker or media management UI. Published Preview displays current Guest media; Draft Preview returns no refs/routes and shows a safe hint. | OWNER/MANAGER for own venue; Guest receives guest-safe proxy URL only. | `VenueInfoSectionsRepository.kt`, `VenueInfoSectionMediaRepository.kt`, `TelegramBotRouter.kt`, `GuestVenueRoutes.kt`, `GuestVenueReadService.kt`, `guestVenue.ts` | High until storage/bridge, validation, deletion and audit semantics are specified. | Public-card media: **PARTIAL / BOT-FIRST**. Venue Mini App upload: **MISSING / FUTURE**. | **NEEDS_MEDIA_STORAGE_SPEC_FIRST** before a runtime slice. |
...
- Historical M1 gate: do not show a feature until real screens/routes exist. `Статистика` later satisfied this gate after M2; `Предпросмотр карточки` now satisfies it for Published Phase 1 and saved-DRAFT Phase 2.1.
...
- Published Guest Preview Phase 1 is **DONE / MVP / STAGING-SMOKE-PASSED**: OWNER/MANAGER use the exact Guest-visible read model and unchanged lifecycle/subscription guards.
- Draft Preview Phase 2.1 is **DONE / MVP / STAGING-SMOKE-PASSED**: a separate OWNER/MANAGER-only Venue route/read model exposes saved `DRAFT` public-candidate data without a Guest API bypass.
- One global and one public-card-settings entry open the same read-only `Предпросмотр карточки` renderer.
- Draft visible info sections are text-only; one compact hint says media will be available after publication, and no raw media refs or new media route are exposed. Published media is unchanged.
- HIDDEN/PAUSED/SUSPENDED/ARCHIVED remain safe unavailable states; DELETED/missing fail safely. Unsaved form state, versioned snapshots, publish actions, mutations and public/share links remain out of scope.

Definition of Done:

- Published Preview matches Guest Mini App visibility rules exactly.
- Draft Preview preserves sub-entity visibility and exposes no private staff/order/billing/provider/audit data.
...
| --- | --- | --- | --- | --- |
| Public card section image/PDF | Add multiple, delete attachment, hide/show whole section; Telegram `file_id`; no direct replace/per-file hide. | View-only through Published Preview; no author/upload/manage flow. | Guest Bot sends Telegram media; Guest Mini App/Published Preview use guarded proxy URLs. | Safe shared source/bridge, validate, replace, visibility, delete, audit, guest-safe URL. |
...
| Photo/PDF menu | Flat `section_type=menu` info section; add/delete and whole-section visibility. | No management; Published view only; Draft safe hint/no refs. | View-only pre-QR info; never orderable by itself. | Manage through the same foundation without merging it with structured menu. |
| Staff profile photo | No supported upload/manage flow. | `Фото сотрудника — позже`; raw manual input hidden. | Initials placeholder; Draft omits photo ref. | Separate employee-consent, moderation, replacement/deletion and approved delivery rules. |
