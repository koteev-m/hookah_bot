...
Status: code/e2e-backed; manual Telegram group smoke remains required.

Scope:

- Reuse existing backend/bot staff-chat semantics: status, link-code, outbox-backed test message and owner-only unlink.
- Venue Mini App `Чат персонала` shows linked/unlinked state, masked chat id, full link code/command immediately after generation, active-code hint after reload, and clear Telegram group instructions.
