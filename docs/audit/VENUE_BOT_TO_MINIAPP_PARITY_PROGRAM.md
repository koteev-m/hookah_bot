...
| A. Work shift / bookings | Implemented/partial. Bot has `/my` active booking list/actions, confirm/cancel/change/message, seated/no-show, hold settings. | Implemented: booking list/status/message routes, expiry/reminder repository foundation, booking hold settings route using `venue_booking_settings.hold_minutes`, and Guest account-level active booking route. | Implemented: Venue booking list/actions, `Написать гостю`, seated/no-show, venue-local display, hold deadline display, M7a `Настройки брони`, and M7b Guest Mini App `Мои брони`. | `BOOKING_VIEW`; `BOOKING_ARRIVAL_UPDATE`; `BOOKING_MANAGE` for venue management/message/settings actions. Guest list is user-scoped. STAFF view/arrival only; settings stay OWNER/MANAGER. | `GuestBookingRoutes.kt`, `GuestBookingRepository.kt`, `VenueBookingRoutes.kt`, `guestBookings.ts`, `venueBookings.ts`, `venueSettings.ts`, `TelegramBotRouter.kt` | Medium: runtime reminder worker and preorder remain later; keep public display number parity in regression. | M3 closed; M7a CLOSED; M7b implemented / staging smoke target; M7c reminders next. | Booking route/RBAC tests, Mini App booking/settings e2e, Telegram `/my` runtime smoke. |
...
- M7b implemented / staging smoke target: Guest Mini App `Профиль → Мои брони` lists active/upcoming bookings across venues with bot-compatible public labels, venue-local time/deadline fields and guest change/cancel actions.
- M7c documented next: adaptive transactional booking reminders, one reminder max per confirmed/changed booking in MVP, no runtime worker/migrations in M7b.
...
- Guest booking parity is not marked closed until both Bot `/my` and Guest Mini App `Мои брони` use the same public booking labels, venue-local display/deadline fields and supported guest actions.
