...
| A. Work shift / bookings | Implemented/partial. Bot has booking list, confirm/cancel/change/message, seated/no-show, hold settings. | Implemented: booking list/status routes, reminder/expiry workers in backend. | Implemented: booking list, confirm/cancel/change, seated/no-show, venue-local display, hold deadline display. | `BOOKING_VIEW`; `BOOKING_ARRIVAL_UPDATE`; `BOOKING_MANAGE` for management actions. | `VenueBookingRoutes.kt`, `GuestBookingRepository.kt`, `venueBookings.ts`, `TelegramBotRouter.kt` | Medium: booking settings/reminder controls are not Mini App parity yet. | M3 implemented; keep in regression and stage smoke. | Booking route/RBAC tests, Mini App booking e2e, Telegram runtime smoke. |
...
Status: implemented / local smoke passed; staging smoke still required.
...
### M4 — Promotions Read-Only Summary
...
### M7 — Staff Chat Diagnostics And Unlink
...
### M8 — Menu Semantic And Media Polish
