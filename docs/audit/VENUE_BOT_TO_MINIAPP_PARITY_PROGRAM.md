...
| B. Settings / venue profile-card | Implemented in bot: profile/card fields, address, contacts, description/info sections, hours. | Partial: public-card route covers structured location/contact/description; Yandex geodata proxy is config-gated; guest venue read models expose display address and coordinate route URL; no broad settings API for hours/media. | Partial / M8b code-e2e backed: current `settings` screen manages booking hold, paid shift extension and structured public card basics with dirty/save/focus UX; broader profile/hours/media settings remain bot-canonical. | OWNER/MANAGER only for public card/location provider routes; STAFF hidden/forbidden. | `VenueRepository.kt`, `VenueRoutes.kt`, `GuestVenueRoutes.kt`, `TelegramBotRouter.kt`, `venueSettings.ts`, `guestVenue.ts` | Medium: live Yandex credentials/package and route destination need staging smoke; broad settings still high risk if expanded. | Keep M8b in staging regression; continue future settings only as small backend-supported slices. | Route/RBAC/provider tests, Mini App e2e, manual Yandex suggestion/route smoke. |
