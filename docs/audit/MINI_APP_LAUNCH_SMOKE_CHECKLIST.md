...
- M9b Venue Working Hours and Date Exceptions Mini App Parity is implemented/code-test-e2e-backed, but not yet staging-smoked: OWNER/MANAGER can manage weekly hours and date exceptions in Venue Mini App, STAFF is hidden/forbidden, guest catalog/card read models expose safe today schedule/open state, and direct Guest Mini App booking create/update validates against configured venue hours. Missing schedule setup shows `График не указан` / `Заведение пока не настроило график бронирования.`, not `Закрыто`.
...
- M9b Venue Working Hours and Date Exceptions Mini App Parity needs staging smoke before closure; local backend focused tests, Mini App build and browser e2e passed, but no staging evidence is recorded yet.
...
5. OWNER adds a closed date exception and an open override, then deletes one override.
6. MANAGER repeats a safe edit.
7. STAFF cannot see schedule controls, and direct schedule API calls are forbidden.
8. Guest public catalog/card shows today's open/closed state and today's hours for the published visible venue.
9. Guest booking in valid hours succeeds.
10. Guest booking on a closed weekday, closed override or outside configured hours is rejected.
11. A venue with no configured weekly/date schedule shows `График не указан`, and guest booking returns `Заведение пока не настроило график бронирования.`
12. Bot booking slot behavior still matches the same schedule.
13. A user from another venue cannot read or mutate private schedule settings.
