...
- M8b-Free Venue Mini App structured public profile/card settings is code/test/e2e-backed: OWNER/MANAGER can edit guest-facing country/city/address, public contact and short card description without a runtime geodata provider; country/city suggestions are local, address is manual, STAFF is hidden/forbidden, and guest public venue card/catalog read models plus route links reflect saved fields. Yandex adapters remain optional/commercial-only and disabled by default. Staging smoke is still pending.
