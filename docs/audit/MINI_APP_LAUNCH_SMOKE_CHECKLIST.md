...
- M9b Venue Working Hours and Date Exceptions Mini App Parity plus M9b.1 date-exception ranges/rejection copy and M9b.2 exception save/list UX is implemented/code-test-e2e-backed, but not yet staging-smoked: OWNER/MANAGER can manage weekly hours, inclusive closed/special-hours exception ranges and optional guest-facing reasons/comments in Venue Mini App; successful exception saves close/reset the form and reveal the saved row in the compact list; STAFF is hidden/forbidden; guest catalog/card read models expose safe today schedule/open state; and direct Guest Mini App booking create/update validates configured venue hours with human schedule errors. Missing schedule setup shows `График не указан` / `Заведение пока не настроило график бронирования.`, not `Закрыто`.
...
- M9b/M9b.1/M9b.2 Venue Working Hours and Date Exceptions needs staging smoke before closure; local backend focused tests, Mini App build and browser e2e passed for the implemented behavior, but no staging evidence is recorded yet.
...
8. MANAGER repeats a safe edit.
9. STAFF cannot see schedule controls, and direct schedule API calls are forbidden.
10. Guest public catalog/card shows today's open/closed state and today's hours for the published visible venue.
11. Guest booking in valid hours succeeds.
12. Guest booking on a closed weekday, closed override or closed period is rejected with human copy; if a reason exists, the guest sees it and `Выберите другую дату.`
13. Guest booking outside configured hours is rejected with human copy showing the effective hours for that day when known.
14. A venue with no configured weekly/date schedule shows `График не указан`, and guest booking returns `Заведение пока не настроило график бронирования.`
15. Bot closed-date selection shows the reason when present and offers `📅 К выбору дат` plus `🏠 В каталог`; ordinary slot behavior still matches the same schedule.
16. A user from another venue cannot read or mutate private schedule settings.
