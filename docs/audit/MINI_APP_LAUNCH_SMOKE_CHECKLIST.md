...
- M5 staff calls lifecycle is CLOSED after staging smoke: Guest Mini App uses a transient compose modal and compact NEW/ACK/DONE/CANCELLED status, Venue Mini App `Вызовы` active queue supports accept/close for NEW/ACK, and backend/staff-chat callbacks share the lifecycle.
...
- Venue Mini App Guest Preview Phase 2.1 is **VENUE MINI APP GUEST PREVIEW / PUBLISHED + PRIVATE DRAFT READ-ONLY / DONE / MVP / STAGING-SMOKE-PASSED**. One `Предпросмотр для гостя` entry calls `GET /api/venue/{venueId}/guest-preview`; the backend selects exact guarded `PUBLISHED_PUBLIC` Guest state or OWNER/MANAGER own-venue `PRIVATE_DRAFT` saved public projection through the shared public-facing assembly. Focused backend preview/Guest/RBAC/promotion tests, Kotlin compile/lint, Mini App build and deterministic smoke `95/95` passed; GitHub Actions were green, staging deploy completed and manual staging smoke passed. This slice did not add or validate media upload.
- Menu Shift Check Phase 1 is **MENU SHIFT CHECK PHASE 1 / DONE / MVP / STAGING-SMOKE-PASSED**.
  OWNER/MANAGER prepare item/option availability in a local draft and confirm one optimistic atomic
...
- Guest Favorites Phase 1: DONE / MVP / STAGING-SMOKE-PASSED. Venue-only favorites support catalog/detail add/remove and Account `Избранные заведения`; current-user isolation, unavailable filtering/restoration, Bot/Mini App shared storage, Telegram Profile/Catalog entrypoints and source-aware Back are covered. Favorite menu items/options, recommendations/frequent items, notification opt-in, favorites-based promotions and loyalty remain future.
- Repeat as Template Phase 1: MVP IMPLEMENTED / LOCAL VALIDATION PASSED / DEFERRED MANUAL SMOKE. Guest History exposes one action per completed order, Mini App shows current-price eligible/skipped preview, and explicit confirmation adds only eligible quantity/options/note to the local cart. The authenticated repeat-plan API and Telegram share one `RepeatOrderResolver`; same-venue active table context, authorized personal/joined shared tab, current menu/options/availability/prices and venue availability are server-enforced. No persistent template, order, batch or staff-chat notification is created. Environment-dependent checks remain `BLOCKED_BY_ENVIRONMENT` in [`REPEAT-MANUAL-001`](../DEFERRED_MANUAL_SMOKE_BACKLOG.md#repeat-manual-001).
- Simple Venue Promotions Phase 1: DONE / MVP / STAGING-SMOKE-PASSED. Venue Owner/Manager can list, create, edit, activate, pause and archive informational promotions; Staff cannot see/open the section and is denied by the API. Guest venue detail shows only current `ACTIVE` promotions for a guest-available venue. Telegram and Mini App share `VenuePromotionRepository`; no migration, discount engine or notification path was added. GitHub Actions were green, staging deploy completed and manual smoke passed.
- Executable Promotions Phase 2 / Happy Hours Percent: DONE / STAGING-SMOKE-PASSED. Staging covered creation/activation validation, weekday/time windows, item/category targets, current price and selected-option delta, preview, submit recalculation, persisted bill/History, no stacking, manual-discount rejection, Owner/Manager/Staff RBAC, Bot/Mini App parity and `TEXT_ONLY` regression.
- Promotion creation audit: `DANGEROUS ACTION AUDIT SLICE / PROMOTION CREATION AUDIT / DONE / MVP / STAGING-SMOKE-PASSED`. Mini App/Telegram creation parity, informational and initial-rule projections, exactly-one `VENUE_PROMOTION_CREATED`, rollback and payload privacy passed the bounded smoke; configuration edit, media/banner and broader audit remain future.
- Promotion effective state clarity: `PROMOTION EFFECTIVE STATE CLARITY / DONE / MVP / STAGING-SMOKE-PASSED`. Lifecycle status remains authoritative; scheduled/current/expired management presentation, expired Guest/pricing exclusion and extension through existing update plus authoritative reload passed the bounded smoke.
- Gift parity: `GIFT_WITH_ITEM BOT/MINIAPP PARITY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT`. Required backend selectors, Kotlin lint/compile, Mini App build and deterministic e2e `83/83` passed; independent review, CI and staging smoke remain open.
- Order/session/tab core docs: `docs/ORDER_SESSION_TAB_CORE.md` is the source of truth for `TABLE_SESSION`, `ACTIVE_TABLE_ORDER`, `ORDER_BATCH`, `TAB`, bill/request/close flow, privacy boundaries and visit-history foundation. Current runtime docs say table-session/tab scoping, Guest History Foundation and Post-Visit Feedback MVP are closed; Repeat Phase 1 is local-smoke-passed; force-close reason/audit, broader loyalty/preorder and analytics remain future/partial.
...
- Venue Mini App Guest Preview unified smoke passed after green Actions and staging deploy: OWNER/MANAGER published preview matched the real Guest card under unchanged Guest availability/subscription guards; private preview exposed only saved public-facing state for allowed unavailable lifecycle states and remained unreachable from public Guest APIs; STAFF/foreign access was denied; dirty settings blocked without auto-save; preview contained no Guest mutations; origin-aware return and two-venue stale-response isolation passed; real Guest actions and Venue Mode sections remained unchanged.
...
- guest growth/retention remains `SPEC UPDATED / PARTIAL-FUTURE` overall in `docs/GROWTH_RETENTION.md`: Guest History Foundation, Post-Visit Feedback MVP, venue-only Guest Favorites Phase 1, Simple Venue Promotions Phase 1, Happy Hours Percent, Promotion Creation Audit and Promotion Effective State Clarity are closed/staging-smoked; Repeat as Template Phase 1 is locally validated with deferred manual smoke in `REPEAT-MANUAL-001`; Gift parity is `GIFT_WITH_ITEM BOT/MINIAPP PARITY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT` and awaits independent review, CI and staging. Favorite menu items/options, recommendations/frequent items, persistent templates, notification opt-in, favorites-based promotions and loyalty remain future smoke targets;
...
- `VenueMenuRoutesTest` / `VenueMenuRepositoryTest`
  - OWNER/MANAGER own-venue `MENU_SHIFT_CHECK`, STAFF/foreign denial and strict request shape;
  - combined 500-change limit, duplicates, missing/foreign ids and option/item ownership checks;
  - mixed item/option all-or-nothing persistence, expected-availability stale rejection and no
    process-local lock;
  - successful/no-op completion writes exactly one `MENU_SHIFT_CHECK_COMPLETED` audit in the same
    transaction; invalid/RBAC/rollback paths write none;
  - response returns the saved categories plus changed/reviewed/available counts.
- `VenueGuestPreviewRoutesTest`
  - OWNER/MANAGER `PUBLISHED_PUBLIC` payloads equal the guarded Guest venue/info payloads;
  - `PRIVATE_DRAFT` is selected server-side and includes only own-venue saved guest-facing card,
    schedule/date exceptions, visible info/media, Today Staff and current `ACTIVE` promotions;
  - STAFF/foreign access is forbidden; archived/deleted/missing venues fail safely; Guest routes
    remain closed to unpublished state;
  - hidden sections/media, private markers, unpublished staff, inactive/non-current promotions and
    raw refs are absent;
  - responses and authenticated venue/section/media-scoped delivery are `no-store`.
- `GuestVenueRoutesTest`, `VenueRbacRoutesTest` and `VenuePromotionRepositoryTest` keep unchanged
  Guest availability/RBAC boundaries and prove private preview does not leak inactive or
  non-current promotions.
- Mini App Playwright preview scenarios cover the one entry, server-selected modes, exact
  badges/copy/safe reasons, origin-aware returns, OWNER/MANAGER access, STAFF hidden/direct denial,
  public-card/weekly-schedule/date-exception dirty guards, authenticated private media, stale-free
  cross-mode and same-mode switching including late responses, GET-only traffic independent of
  table/cart/order state, no mutation controls and loading/empty/error/unavailable states.
- Focused Venue Menu Playwright scenarios cover category counts, search and all three filters,
  draft-only toggles, selected/category/item-option mass actions, confirmation summary, cancel with
  no request, exactly one confirm request, recoverable stale refresh, venue-switch cleanup and
  STAFF hidden/direct denial.
...
- OWNER and MANAGER open the one `Предпросмотр для гостя` entry. For `PUBLISHED_PUBLIC`, compare
  card/location/contact/description, schedule/date exceptions, info/media, Today Staff and active
  promotions with the real Guest card; confirm `Опубликовано`,
  `Так карточку сейчас видит гость.` and `Вернуться в кабинет`.
- Open from public-card Settings and confirm the same renderer returns through
  `Вернуться к настройкам`.
- For `PRIVATE_DRAFT`, confirm `Черновик`,
  `Гости пока не видят эту карточку. Это закрытый предпросмотр сохранённой версии.`, the saved
  guest-safe projection, authenticated visible media, published Today Staff/current `ACTIVE`
  promotion filters and no raw refs.
- Confirm the private reason is only `Заведение ещё не опубликовано.`,
  `Заведение временно скрыто.` or `Заведение приостановлено.`, with no technical subscription
  status. Archived/deleted/missing venues fail safely.
- Change public-card, weekly-schedule and date-exception values without saving. Each case must stay
  in Settings, send no preview/save request and show
  `Есть несохранённые изменения. Сначала сохраните их, затем откройте предпросмотр.`. Explicitly
  save and confirm preview reads the new values.
- Switching venue while preview is loading across `PUBLISHED_PUBLIC`/`PRIVATE_DRAFT`, including
  same-mode switches, clears the old venue immediately and cannot restore a stale response.
...
Guest Preview Phase 2.1 result:

- **VENUE MINI APP GUEST PREVIEW / PUBLISHED + PRIVATE DRAFT READ-ONLY / DONE / MVP /
  STAGING-SMOKE-PASSED**.
- Focused `VenueGuestPreviewRoutesTest`, `GuestVenueRoutesTest`, `VenueRbacRoutesTest` and
  `VenuePromotionRepositoryTest` selectors passed; Kotlin compile/lint and Mini App production
  build passed; deterministic browser smoke passed `95/95`.
- Exact Guest parity, saved-private allowlist, OWNER/MANAGER own-venue RBAC, STAFF/foreign denial,
  dirty-state/no-auto-save handling, scoped authenticated media, private-field/raw-ref absence,
  closed Guest routes, no Guest actions and stale-response isolation passed locally and on staging.
- GitHub Actions were green, staging deploy completed, and manual smoke confirmed both backend-
  selected modes, saved-state semantics, origin-aware navigation, two-venue switching and unchanged
  real Guest/Venue Mode regressions.
- Venue Mini App media upload/manage was not exercised and must not inherit the Preview status.
...
### Menu Shift Check Phase 1 smoke status

Status: **MENU SHIFT CHECK PHASE 1 / DONE / MVP / STAGING-SMOKE-PASSED**.

Passed staging/manual coverage:

1. OWNER and MANAGER see two compact task-oriented accordions: `Редактирование меню` and
   `Проверка меню перед сменой`; both start collapsed and opening one collapses the other.
2. The new-category form stays hidden until `Добавить категорию`; categories remain compact and
   expand on demand.
3. Shift-check description says changes apply only after confirmation; readiness category/item/
   option counts match saved backend state, and search plus filters work.
4. Normal mode has no selection checkboxes: item/option availability uses the clear `В наличии` /
   `Нет в наличии` switch, options are visually nested and dirty rows show `Изменено`.
5. Availability switches and selection controls use distinct roles/accessible labels.
6. Selection checkboxes appear only after `Массовое изменение`; selected count, make available,
   make unavailable, clear selection and exit actions change only the local draft.
7. Collapse/reopen preserves the draft. Cancel restores saved backend state without mutation or
   completion audit.
8. Confirmation summary shows correct available/unavailable item and option counts; a mixed confirm
   sends exactly one `POST /api/venue/menu/shift-check?venueId=<id>` and refreshes authoritative
   state.
9. Exactly one safe `MENU_SHIFT_CHECK_COMPLETED` audit contains actor/venue ids, changed/reviewed
   counts and bounded ids, with no names, prices, raw Telegram/initData/private data or full body.
10. A no-op review changes no menu row and creates exactly one audit with zero changed counts.
11. Stale Owner/Manager state rejects the whole confirmation with
    `Меню изменилось. Обновите проверку и повторите подтверждение.`, with no partial write/audit.
12. Duplicate, missing, foreign, option/item mismatch and oversized requests apply no partial
    writes and create no completion audit.
13. Venue switch resets expanded state, draft, selection and pending confirmation context; no old
    response can affect the new venue.
14. STAFF has no entry and direct API is forbidden; existing individual item/option availability
    controls and Telegram Bot stop-list still work.
15. Confirmed state reaches Guest menu; stale item/option carts fail preview/add-batch validation.
16. Menu/order regression and cleanup of staging test data passed.
...
- Venue/public-card media management is `PARTIAL / BOT-FIRST`: Bot OWNER/MANAGER can add image/PDF
  attachments, delete an attachment and hide/show its whole info section; there is no direct
  replace or per-attachment hide action. Venue Mini App has no file picker, upload endpoint or
  authoring/manage flow.
- Structured menu item photo/thumbnail/description media is not implemented in current Bot,
  Venue Mini App or Guest DTO/rendering. This is separate from the working view-only
  `📖 Фото-меню` info section.
- Current smoke has no media-upload success claim. Add upload/replace/hide/delete smoke only after
  the future media foundation is implemented.
...
Use the History, Feedback, Favorites and Promotions sections after changes to their implemented slices. Current overall status is `SPEC UPDATED / PARTIAL-FUTURE` in `docs/GROWTH_RETENTION.md`, with Guest History Foundation, Post-Visit Feedback MVP, venue-only Guest Favorites Phase 1, Simple Venue Promotions Phase 1, Happy Hours Percent, Promotion Creation Audit, Promotion Effective State Clarity, Promotion Lifecycle Status Audit and Venue Promotions Current/Archived Tabs UX already `DONE / STAGING-SMOKE-PASSED`. Repeat is locally validated, but its environment-dependent manual scenarios are not passed. Gift parity is `GIFT_WITH_ITEM BOT/MINIAPP PARITY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT`; keep it in local/CI regression and run independent review and staging smoke before release.
...
5. Missing `promotionDiscounts`, options or note does not crash.
...
10. Real 404 shows `Не удалось загрузить детали истории.`.
11. Foreign detail returns 404 and privacy checks remain strict.
12. Guest does not see another guest's personal tab/order detail.
...
- status: `MVP IMPLEMENTED / LOCAL VALIDATION PASSED / DEFERRED MANUAL SMOKE`;
- current manual status: `BLOCKED_BY_ENVIRONMENT`;
- canonical prerequisites, exact scenarios, expected results, cleanup and closure criteria:
...
Promotion effective state clarity: **PROMOTION EFFECTIVE STATE CLARITY / DONE / MVP / STAGING-SMOKE-PASSED**.
...
Current effective-state correction: the bounded staging smoke passed. The database lifecycle status
was not rewritten; `DRAFT`, `PAUSED` and `ARCHIVED` retained priority, while `ACTIVE` was
`Запланирована` before start, `Действует сейчас` inside the inclusive period and `Период завершён`
after end. Expired stayed in `Текущие`, was absent from Guest and ineligible for pricing. Extension
used the existing update plus authoritative reload, with no automatic pause/archive, worker, system
actor or lifecycle audit. Live open-screen time-boundary refresh, invalid-timestamp fail-safe UI,
exact-boundary fixtures and the duplicate extension/edit action decision remain future.
...
Current creation correction: **DANGEROUS ACTION AUDIT SLICE / PROMOTION CREATION AUDIT / DONE /
MVP / STAGING-SMOKE-PASSED**.

- Action is `VENUE_PROMOTION_CREATED`, entity is `venue_promotion`, actor is server-derived and
  source is `VENUE_MINI_APP` or `TELEGRAM_BOT`.
- Parent, caller-connection initial rule and audit share one transaction; one committed parent
  produces exactly one creation audit.
- Informational creation uses `rules=[]`; Mini App Happy Hours/Gift records the actually created
  initial rule; Telegram Happy Hours/Gift parent-draft creation uses `rules=[]`.
- Banner media persists separately and is absent from the creation payload.
- Denial, validation, `afterInsert`, SQL and audit failure produce no success audit; audit failure
  rolls back parent and initial rules.
- Payload excludes promotion text/config/prices/media, Telegram PII and unrelated PII.

The bounded staging smoke passed for Mini App/Telegram parity, rule projection, exactly-one audit,
failure rollback and privacy. Configuration edit, schedule/target/reward, media/banner audit, Banner
retry duplicate-draft UX and broader dangerous-action audit remain future.
...
5. Owner edits and reactivates it; both Venue and Guest surfaces show the current state.
6. Manager management actions match Owner operational policy; Staff has no navigation entry and direct API access is forbidden.
7. Future, expired and archived promotions stay hidden from Guest.
8. Hidden/paused/suspended/archived/deleted or subscription-blocked venues disclose no promotion data.
9. Mini App-created promotion is visible through the existing Telegram guest flow; Telegram-created informational `TEXT_ONLY` promotion is visible in Venue Mini App and Guest venue detail. Rule-backed templates remain managed by their existing Telegram flows.
10. No order total, automatic discount, batch, staff-chat message or marketing notification is created.

Executable Promotions Phase 2 / Happy Hours Percent
(`DONE / STAGING-SMOKE-PASSED`; keep in regression):

1. Bot and Mini App call one shared backend resolver and receive the same rule/version, affected
   lines, adjustment and total for the same cart fixture.
2. Venue date range, timezone, weekdays and different per-day time windows are evaluated
   server-side at preview and submit.
3. Current menu item/category eligibility, availability, selected-option price delta and current
   price are revalidated; the client submits no trusted discount amount.
4. Guest sees ordinary price, named promotion adjustment and final total before explicit
   confirmation; preview creates no order or batch.
5. A time, availability, price or cart change between preview and submit causes a safe recalculation.
6. One non-stackable percentage promotion is applied; manual discount does not stack without an
   explicit tested policy; total cannot become negative.
7. The persisted application snapshots promotion/rule/version and affected-line original,
   adjustment and final amounts; History remains stable after later config/menu edits.
8. Repeated idempotent submit creates no second application or discount.
9. Excluded, canceled and rejected lines do not retain an executable promotion; personal/shared
   tab and cross-venue privacy remain intact.
10. Staff-chat displays persisted order facts but contains no eligibility or pricing calculation.

Confirmed staging evidence additionally includes creation/activation validation,
Owner/Manager/Staff RBAC, Bot/Mini App parity and `TEXT_ONLY` regression. Do not reopen the
percentage preset into a universal reward form.

GIFT_WITH_ITEM Bot/Mini App parity
(`GIFT_WITH_ITEM BOT/MINIAPP PARITY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT`;
independent review and staging smoke pending):

1. Venue Owner/Manager creates a separate `Подарок при покупке` draft with date range,
   venue-timezone weekday/time windows and one item/category trigger; Staff remains hidden/denied.
2. Fixed reward and selectable menu-item allowlist activate only after server validation.
3. Bot and Mini App receive the same fixed/selectable/unavailable offer, signed decision scope,
   rule/version, trigger and allowlist from one repository-backed resolver/fixture.
4. Fixed gift requires visible acceptance; selectable gift requires one explicit allowlist choice;
   both clients expose `Пропустить подарок`.
5. Preview creates no order, batch, reward, application, adjustment, reward link or staff-chat
   event. Its 10-minute HMAC scope uses `gift_decision/v1` domain separation, purpose
   `gift_decision` and audience `hookah-order-submit`.
6. The signed scope binds user, venue, table session, tab, canonical cart fingerprint,
   promotion/rule/version and offer type without trusted financial facts. Submit verifies it, then
   revalidates venue time, lifecycle, schedule, trigger/required options, current availability and
   price, allowlist, session/tab membership, one-gift winner and idempotency.
7. A trigger removed/rejected/canceled/excluded before authoritative persistence cannot create the
   gift. After submit, cancel-as-unavailable on the trigger atomically cancels its active linked
   reward; trigger exclusion atomically excludes it. Repeat operations are idempotent, an already
   inactive reward is not restored, and reward-only mutation never cancels the trigger.
8. An unavailable fixed reward, unavailable selected reward and all-unavailable allowlist show
   explicit recovery/unavailable copy.
9. One gift maximum is persisted for the submitted batch/tab; multiple trigger quantities or
   multiple eligible rules do not multiply rewards and there is no cross-batch accumulation.
10. Reward line shows original amount, named 100% adjustment and final zero; trigger/reward link,
    selected reward item and rule/version snapshots remain in DB, active bill and History after
    coupled cancellation/exclusion.
11. Reward line receives neither percentage nor manual discount. Its linked trigger also rejects
    manual discount while the reward is active with
    `На эту позицию уже действует акция. Ручную скидку применить нельзя.` STAFF sees no Telegram
    discount action, and direct/stale-dialog attempts are denied before repository mutation.
12. Price/cart/config or user/venue/session/tab change produces a safe recalculation, persists
    nothing partially and shows `Корзина изменилась. Проверьте подарок ещё раз.`
13. Duplicate submit leaves order, batch, item, application, adjustment and reward-link counts
    unchanged.
14. Mini App LocalStorage is scoped by user, venue, table session, tab, cart fingerprint and expiry;
    initial no-previous-tab restore, account/venue/session/tab changes, same-QR session replacement
    and cart item/quantity/option/note changes clear it.
15. Telegram callbacks contain an amount-free per-offer tag. Restart/process-memory loss, missing
    binding and an old callback from another session/tab fail safely; serialized fixed/selectable/
    skip decisions are revalidated through a fresh resolver/router and production repository path.
16. Coupled mutations use deterministic order/batch → trigger → linked reward → link/application
    locking and bill recalculation in one transaction. Injected failure rolls back both lines,
    totals and History; Guest bill, Venue bill, History and staff-chat read the same committed facts.
...
`VisitRepository` final 16/0 after one detected/fixed initial failure, `TelegramBotRouter` 503/0
and real PostgreSQL concurrency 6/0 with `JAVA_TOOL_OPTIONS=-Dapi.version=1.44`, all with
skipped=0. Kotlin `ktlintCheck`/`compileKotlin`, Mini App build and deterministic Playwright smoke
`83/83` passed. Before release, require independent review, green GitHub Actions and staging checks
for Owner/Manager/Staff RBAC, real Telegram fixed/select/skip/restart behavior, Mini App
cart/history/bill, unavailable recovery, current-price recalculation, idempotency and Happy
Hours/`TEXT_ONLY` regression.

BOGO, X+Y, second-item-free, free option/refill, fixed discount, special price, loyalty, promo
codes, cross-visit/cross-batch accumulation, notifications, paid placement, payments/Stars/crypto
and an arbitrary rule builder remain outside this target.

Remaining Future Growth:
1. Favorite menu items/options, recommendations and frequent items remain absent.
2. Persistent repeat-template library remains absent.
3. Promo/retention notifications require opt-in and can be disabled.
4. Platform paid placement label is visible if/when paid placement is implemented.
...
App build and full deterministic smoke `100/100` passed locally. GitHub Actions were green,
staging deploy completed and the manual functional/UX checks above passed.

Simple Venue Promotions Phase 1, Executable Promotions Phase 2 / Happy Hours Percent, Promotion Creation Audit and Promotion Effective State Clarity are DONE / STAGING-SMOKE-PASSED. Gift parity is `GIFT_WITH_ITEM BOT/MINIAPP PARITY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED / REVIEW REQUIRED BEFORE COMMIT`, bounded to signed scoped decisions, explicit fixed/selectable gift choice, optional skip, one gift per submitted batch/tab, current server revalidation, coupled trigger/reward lifecycle and persisted linkage through the existing engine; its next gates are independent review, green CI and staging cross-surface smoke. Repeat as Template Phase 1 remains MVP IMPLEMENTED / LOCAL VALIDATION PASSED / DEFERRED MANUAL SMOKE, and `REPEAT-MANUAL-001` remains open unchanged. Existing order/session/tab uniqueness and privacy behavior stays in regression rather than being reopened without evidence; M4A-M4C messages, M5 staff calls, M6 staff-chat management, M7b, rollout-gated M7c, M8b-Free, M9a, M9b/M9b.1/M9b.2/M9b.3, Guest History, Post-Visit Feedback and Favorites stay in regression smoke.
...
Manual staff-call lifecycle regression smoke after deployment:

1. Guest creates staff-call from active table context.
2. Staff/Manager/Owner accepts and completes the call from Venue Mode or an allowed staff-chat shortcut.
3. Guest staff-call status shows terminal `DONE`.
4. Auto-cancelled/`CANCELLED` call appears to the same guest in the current table session as `Вызов отменён`.
5. Venue active queue shows only active `NEW` / `ACK` calls and does not show `CANCELLED`.
6. Guest does not see another guest's or another `tableSessionId`'s `CANCELLED` call.
7. Support tickets and ordinary venue chats still do not post to staff-chat.
