...
Guest communication follows `docs/COMMUNICATION_MODEL.md`: Owner/Manager handle `BOOKING_CHAT`, `VENUE_CHAT` and own-venue `SUPPORT_TICKET`; Staff does not handle support/venue chats; `STAFF_CALL` remains operational.
...
15. Owner/Manager can create a simple promotion with title, description, active period, terms and visibility/status.
16. Promotion is visible to guests only during the active period and not visible when hidden/suspended.
17. Promotion copy does not imply automatic discount unless the promo engine is implemented.
