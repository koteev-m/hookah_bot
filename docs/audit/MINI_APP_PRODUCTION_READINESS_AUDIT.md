...
>
> Current correction as of 2026-07-23: Guest Favorites Phase 1 is DONE / MVP / STAGING-SMOKE-PASSED for favorite venues only. Mini App catalog/detail add/remove, Account venue-only list/actions/empty state, current-user isolation, unavailable venue filtering with preserved rows and republish restoration are verified. Telegram Catalog and Profile open the same `GuestFavoritesRepository`/`guest_favorite_venues` source; Back returns to the originating screen; Bot and Mini App changes are mutually visible. Historical statements below that favorites/account actions are missing or future are superseded for this venue-only scope. Favorite menu items/options, recommendations/frequent items, repeat templates, notification opt-in, favorites-based promotions and loyalty remain future.
>
...
>
> Current docs correction as of 2026-07-23: Growth/retention source of truth is `docs/GROWTH_RETENTION.md`. Guest History Foundation, Post-Visit Feedback MVP and venue-only Guest Favorites Phase 1 are DONE / MVP / staging-smoke-passed; repeat templates, simple venue promotions and opt-in notifications remain `SPEC UPDATED / PARTIAL-FUTURE`; favorite menu items/options, Platform feedback dashboard, automated review prompts/public review automation, promo codes, loyalty stamps/points, referrals, paid placement/boosting, segmentation and advanced recommendations remain future.
>
...
- Growth/retention MVP from `docs/GROWTH_RETENTION.md`: Guest History Foundation, Post-Visit Feedback and venue-only Guest Favorites Phase 1 are staging-smoked. Recommended next bounded block is Repeat as Template Phase 1; favorite menu items/options, recommendations/frequent items, simple venue promotions, notification opt-in, favorites-based promotions and loyalty remain future.
