...
>
> Current docs checkpoint as of 2026-07-09: Guest growth/retention is consolidated in `docs/GROWTH_RETENTION.md`. Status is `SPEC UPDATED / PARTIAL-FUTURE`: Guest History Foundation is DONE / MVP / staging-smoke-passed, while favorites, repeat templates, post-visit feedback, simple venue promotions and opt-in notifications remain future. MVP terms are `FAVORITE_VENUE`, `VISIT_HISTORY`, `ORDER_HISTORY`, `BOOKING_HISTORY`, `REPEAT_TEMPLATE`, `POST_VISIT_FEEDBACK`, `VENUE_PROMOTION` and `OPT_IN_NOTIFICATION`; promo codes, loyalty stamps/points, referrals, segmentation, paid placement/boosting and advanced recommendations remain future.
>
...
- Guest growth/retention as full cross-channel product flows beyond the now staging-smoked Guest History Foundation: `FAVORITE_VENUE`, `REPEAT_TEMPLATE`, `POST_VISIT_FEEDBACK`, simple `VENUE_PROMOTION` and `OPT_IN_NOTIFICATION`. Several backend/bot baselines may exist, but each complete product loop remains partial/future until Bot + Mini App + backend smoke proves it.
...
2. **P2**: Guest growth/retention MVP from `docs/GROWTH_RETENTION.md`: Guest History Foundation is now closed/staging-smoked; recommended next bounded runtime block is post-visit feedback MVP from completed History. Favorites, repeat templates, simple venue promotions and opt-in notification rules remain future.
...
3. Build the next growth feature as one bounded milestone on top of the completed History foundation; current recommendation is post-visit feedback MVP. Keep opt-in notification rules and owner promotion workflow separate unless explicitly scoped.

# Какие функции лучше не трогать пока

- Promotions/referrals/favorites/repeat: keep future until separately scoped; post-visit feedback is the recommended next small block now that visit/order/session history is stable.
...
| Guest growth/retention | SPEC UPDATED / PARTIAL-FUTURE; History DONE / staging-smoke-passed | `docs/GROWTH_RETENTION.md`, account/history/favorites baselines where present | Guest History Foundation is closed and stays in regression; recommended next block is post-visit feedback MVP from completed History. Favorite venues, repeat template, simple venue promotions and opt-in notifications remain future; promo codes, loyalty, referrals, paid placement boosting and segmentation remain future |
