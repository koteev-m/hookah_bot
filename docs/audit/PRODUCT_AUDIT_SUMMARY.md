...
>
> Current checkpoint as of 2026-06-30: Staff Call Lifecycle ACK/DONE audit hardening is CLOSED / staging smoke passed. Successful applied staff-call ACK/DONE transitions write `STAFF_CALL_ACK` / `STAFF_CALL_DONE` audit evidence from both Venue Mini App routes and Telegram staff-chat callbacks with top-level `actorUserId`, venue/call/status/source payload and no guest comment/display-name or secrets. Staging confirmed guest-created calls, Venue Mini App ACK/DONE audit rows with `source=venue_miniapp`, Telegram staff-chat ACK/DONE callback edits plus audit rows with `source=telegram_staff_chat`, and guest ability to create a new call after DONE. Audit remains best-effort after the operational transition; row-level `acked_by` / `done_by` / ACK-DONE timestamp columns, manual cancel UI and broader staff-call UX remain out of scope. Guest-visible `CANCELLED` is closed by the 2026-07-08 checkpoint below.
>
...
>
> Current docs checkpoint as of 2026-07-23: Guest growth/retention is consolidated in `docs/GROWTH_RETENTION.md`. Status remains `SPEC UPDATED / PARTIAL-FUTURE` overall, while Guest History Foundation, Post-Visit Feedback MVP and Guest Favorites Phase 1 are DONE / MVP / STAGING-SMOKE-PASSED. Favorites covers venue-only catalog/detail mutations, Account list/actions/empty state, current-user isolation, unavailable filtering with preserved-row restoration, shared Bot/Mini App storage, Telegram Catalog/Profile entrypoints and source-aware Back. Favorite menu items/options, recommendations/frequent items, repeat templates, simple venue promotions, notification opt-in, favorites-based promotions and loyalty remain future; Platform feedback dashboard, automated review prompts/public review automation, promo codes, loyalty stamps/points, referrals, segmentation and paid placement/boosting remain future.
>
> Current docs checkpoint as of 2026-07-09: Order/session/tab core is consolidated in `docs/ORDER_SESSION_TAB_CORE.md`. Status is `SPEC UPDATED`: `TABLE_SESSION`, `ACTIVE_TABLE_ORDER`, `ORDER_BATCH`, `TAB`, bill/request/close flow, privacy boundaries and visit-history foundation are now the canonical model. Current runtime docs say the old table-only active-order risk and Guest History Foundation MVP are closed; remaining gaps are force-close reason/audit, DB-level uniqueness nuances, repeat/feedback/loyalty/preorder and broader analytics events.
>
...
>
> Current docs checkpoint as of 2026-07-09: Booking lifecycle is consolidated in `docs/BOOKING_LIFECYCLE.md`. Status is `SPEC UPDATED`: guest booking flow, Venue booking queue, state machine, hold minutes, `arrival_deadline`, reminders, `BOOKING_CHAT`, support routing, analytics, RBAC and smoke are now canonical. Current bounded slices cover queue/hold/guest list/chat and booking `SEATED` -> Guest History paths, while reminder rollout, automation, preorder and feedback remain partial/future unless specific implementation evidence exists.
>
...
>
> Current checkpoint as of 2026-07-08: Booking Arrival Guard / Staff-Chat Booking Buttons are CLOSED / staging smoke passed. Backend and Venue Mini App allow `Гость пришёл` / `Не пришёл` only for `CONFIRMED`; `PENDING`, `CHANGED` and terminal statuses do not show or accept seat/no-show; staff-chat booking notifications are state-aware (`PENDING`: confirm/cancel/message, `CONFIRMED`: seated/no-show/cancel/message, `CHANGED`: cancel/message, terminal: no dangerous buttons); stale callbacks answer `Бронь уже изменилась. Откройте кабинет.`, no-permission callbacks answer `Нет прав.`, and `BOOKING_CHAT` replies do not post to staff-chat. No migration was added. Reminder rollout, automatic no-show, preorder and broader visit-history/feedback integration remain partial/future.
>
> Current checkpoint as of 2026-07-08: Staff-call lifecycle finishing patch is CLOSED / staging smoke passed. Guest staff-call status includes `NEW`, `ACK`, `DONE` and `CANCELLED` for the current guest and current `tableSessionId`; `CANCELLED` uses `Вызов отменён`; Venue active queue remains `NEW` / `ACK`; support tickets and venue chats still do not post to staff-chat. No migration, manual cancel UI, Mini App change or staff-chat callback refactor was added.
>
> Current checkpoint as of 2026-07-09: Guest History Foundation MVP staging bugfix is CLOSED / staging smoke passed. History list/detail now show real completed visits/orders only: closed-order visits and booking-only `SEATED` visits. `CANCELED`, `NO_SHOW`, `EXPIRED`, `PENDING` and `CHANGED` bookings are hidden as visits; legacy invalid rows are preserved but filtered. Old closed-order details open safely without required `promotionDiscounts`/options/notes; backend returns `promotionDiscounts: []`; Mini App tolerates missing optional fields. Detail keeps `Не удалось загрузить детали истории.` for real errors, has `← Назад к истории`, and Telegram BackButton returns from detail to History list. Privacy filters remain strict: foreign detail is 404, чужие personal/shared/private details are hidden, and booking `SEATED` + order closed does not double-count when merge/dedup applies. No migration was added.
>
> Current checkpoint as of 2026-07-21: Post-Visit Feedback MVP and its public-review/follow-up smoke-fix are CLOSED / staging smoke passed. Feedback is History-detail-only for own completed visits, including booking-only `SEATED`; manual `5/5` may show an Owner-configured safe Yandex CTA only on explicit click; Owner/Manager sees own-venue feedback and opens exact low-rating `VENUE_CHAT` with context; Staff denied. Bot/Mini App share the Owner-only public review URL source. No automatic Owner message, support ticket, staff-chat notification, worker/Telegram prompt or auto-redirect is enabled.
>
> Current docs checkpoint as of 2026-07-07: Testing/QA smoke strategy is consolidated in `docs/TESTING_QA_SMOKE_STRATEGY.md`. Status is `UPDATED`: local sanity, backend targeted tests, Mini App build/e2e, GitHub Actions expectations, change-type decision matrix, staging policy, failure reporting, manual smoke suites and Codex handoff rules are canonical. CI coverage is release-critical/partial; area-specific manual smoke remains required for runtime-impacting changes.
>
> Current docs checkpoint as of 2026-07-07: Deployment/runbook operations are consolidated in `docs/DEPLOYMENT_RUNBOOK.md`. Status is `UPDATED`: release model, staging deploy command, environment inventory, migration runbook, rollback policy, troubleshooting, incident response and Codex/ChatGPT handoff are canonical. Exact production deploy, previous-image rollback and backup/restore commands remain partial/needs verification.
...
- Booking: bot/backend and Guest/Venue Mini App screens exist; M7a hold settings is closed and M7b Guest Mini App `Мои брони` is implemented with staging visual parity for Bot `/my` label/time/deadline. Confirmed-only arrival actions and state-aware staff-chat booking buttons are staging-smoked. M7c adaptive reminders passed a controlled real Telegram staging smoke with legacy-row reconciliation, truthful `QUEUED` outbox semantics, atomic guest attendance, visible Telegram message editing and cross-channel indicators; runtime remains opt-in disabled for rollout. Preorder remains later.
- Staff calls: create/list/ACK/DONE lifecycle exists across Guest/Venue Mini App and bot/staff chat path, applied ACK/DONE transitions are audited from Venue Mini App and Telegram staff-chat surfaces, and guest-visible `CANCELLED` terminal status is closed for the current guest/tableSession with copy `Вызов отменён`. Staff-chat live order cards now hide DONE/CANCELLED generic calls from active `Оперативно`, and closing an order/bill resolves linked active BILL requests plus closed-visit staff-call leftovers. Manual cancel UI, row-level ACK/DONE actor/timestamp columns and SLA/escalation remain later.
...
- Guest growth/retention beyond the staging-smoked History/Feedback/venue-favorites slices: favorite menu items/options, recommendations/frequent items, `REPEAT_TEMPLATE`, simple `VENUE_PROMOTION`, notification opt-in, favorites-based promotions and loyalty. Each remaining loop stays partial/future until its own Bot + Mini App + backend evidence proves it.
...
2. **P1/P2**: Recommended next bounded runtime block is Repeat as Template Phase 1. Current code already has a Telegram repeat flow plus closed table-session active-order uniqueness/tab-scoping foundations; add a shared safe repeat plan and Mini App cart application without automatic order creation.
...
3. Implement Repeat as Template Phase 1 as the next bounded runtime milestone: current-user completed visit/order, same-venue active table context, selected authorized tab, current menu/options/prices recheck, unavailable-line summary and cart-only application. Keep active-order uniqueness/tab privacy in regression.

# Какие функции лучше не трогать пока

- Favorite menu items/options, recommendations/frequent items, promotions/referrals, notification opt-in and loyalty remain future until separately scoped. Venue favorites are staging-closed; tips/payments/Stars/crypto are not part of Repeat as Template Phase 1.
...
|---|---|---|---|
| Order/session/tab core | SPEC UPDATED / runtime scoping and Guest History closed with partial follow-ups | `docs/ORDER_SESSION_TAB_CORE.md`, `GuestOrderRoutes`, `TableSessionRepository`, `GuestTabsRoutes`, `VenueOrdersRepository` | Keep table-session/tab scoping, personal/shared tab visibility, batch idempotency, bill request, close/expire behavior and Guest History privacy/dedup/filtering in regression; force-close audit, DB-level uniqueness nuances, repeat/feedback/loyalty/preorder and broader analytics remain future/partial |
...
| Testing/QA smoke strategy | UPDATED / CI coverage partial by product breadth | `docs/TESTING_QA_SMOKE_STRATEGY.md`, `.github/workflows/ci.yml`, `docs/audit/MINI_APP_LAUNCH_SMOKE_CHECKLIST.md` | Match checks to change type; use focused backend tests, Mini App build/e2e, GitHub Actions and staging/manual smoke where runtime behavior changes |
| Deployment/runbook operations | UPDATED / rollback and production ops partial | `docs/DEPLOYMENT_RUNBOOK.md`, `docs/STAGING_DEPLOYMENT.md`, `docs/OPERATIONS.md`, `docs/MIGRATION_POLICY.md` | Use canonical release/deploy gates; exact production deploy, previous-image rollback and backup/restore commands need verification |
...
| Staff calls | DONE/PARTIAL | `GuestStaffCallRoutes`, `StaffCallRepository`, `guestVenue.ts`, `venueCalls.ts` | M5 lifecycle, guest-visible `CANCELLED`, notification parity and ACK/DONE audit hardening are CLOSED / staging smoke passed; keep current guest/tableSession scoping, active-only Venue queue and Telegram staff-chat ACK/DONE audit paths in regression |
...
| Menu constructor | SPEC UPDATED / PARTIAL | `docs/MENU_OPTIONS_STOPLIST.md`, `VenueMenuRoutes`, `venueMenu.ts` | Structured options parity is smoke-closed; broader category/item/media/top-list/shift-check governance remains partial/future |
| Stop-list including options/flavors | SPEC UPDATED / PARTIAL | `docs/MENU_OPTIONS_STOPLIST.md`, `TelegramBotRouter` stop-list methods, `VenueMenuRepository` | Current docs say Staff item/option availability parity is aligned; per-venue `staff_stoplist_enabled`, mass stop-list and shift check remain future/partial |
...
| Guest growth/retention | SPEC UPDATED / PARTIAL-FUTURE; History + Post-Visit Feedback + Guest Favorites Phase 1 DONE / MVP / STAGING-SMOKE-PASSED | `docs/GROWTH_RETENTION.md`, shared `guest_favorite_venues` source and current Account/catalog/detail/Telegram evidence | Keep venue favorites, History and Feedback in regression. Repeat as Template Phase 1 is selected next; favorite menu items/options, recommendations/frequent items, simple venue promotions, notification opt-in, favorites-based promotions and loyalty remain future |
...
16. Booking lifecycle regression from `docs/BOOKING_LIFECYCLE.md`: Guest creates/lists/cancels, Venue confirms/proposes/cancels, confirmed bookings can seat/no-show, pending/changed/terminal bookings cannot, hold/deadline copy is correct, booking `Открыть переписку` opens `BOOKING_CHAT`, booking support uses verified context and staff-chat does not receive booking chat/support messages.
...
18. QA/release regression from `docs/TESTING_QA_SMOKE_STRATEGY.md`: validation matches change type, CI failure reports include test/assertion details, docs-only changes skip staging deploy, runtime changes get appropriate area smoke and `scripts/dev/` is not staged accidentally.
19. Deployment/runbook regression from `docs/DEPLOYMENT_RUNBOOK.md`: runtime releases wait for green Actions, staging deploy uses the documented command, smoke starts with health endpoints, migrations include rollback/forward-fix notes and no secrets are exposed in logs/handoff.
20. Guest Favorites Phase 1: add/remove from catalog and venue detail; verify Account venue-only list/actions and empty state; verify two-account isolation; verify hidden/suspended/subscription-blocked filtering without row loss and republish restoration; verify Bot-created favorite in Mini App and Mini App-created favorite in Bot; verify Telegram Profile/Catalog entrypoints and source-aware Back.
21. Owner invite from platform can be accepted end-to-end.
