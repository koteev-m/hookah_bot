...
>
> Current docs checkpoint as of 2026-07-21: Guest growth/retention is consolidated in `docs/GROWTH_RETENTION.md`. Status remains `SPEC UPDATED / PARTIAL-FUTURE` overall, while Guest History Foundation and Post-Visit Feedback MVP are DONE / MVP / staging-smoke-passed. Favorites, repeat templates, simple venue promotions and opt-in notifications remain future; Platform feedback dashboard, automated review prompts/public review automation, promo codes, loyalty stamps/points, referrals, segmentation, paid placement/boosting and advanced recommendations remain future.
>
...
- Guest growth/retention beyond the staging-smoked History and Post-Visit Feedback slices: `FAVORITE_VENUE`, `REPEAT_TEMPLATE`, simple `VENUE_PROMOTION` and `OPT_IN_NOTIFICATION`. Each remaining loop stays partial/future until Bot + Mini App + backend smoke proves it.
...
2. **P2**: Guest growth/retention MVP from `docs/GROWTH_RETENTION.md`: Guest History Foundation and Post-Visit Feedback are closed/staging-smoked; recommended next bounded runtime block is Guest Favorites Phase 1. Repeat templates, simple venue promotions and opt-in notification rules remain future.
...
3. Build the next growth feature as one bounded milestone; current recommendation is Guest Favorites Phase 1: add/remove favorite venue, current-user favorites list and safe hidden/suspended filtering. Keep repeat, opt-in notification rules and owner promotion workflow separate unless explicitly scoped.

# Какие функции лучше не трогать пока

- Promotions/referrals/repeat remain future until separately scoped. Favorites are the recommended next bounded block; loyalty/tips/payments/Stars/crypto are not part of it.
...
| Guest growth/retention | SPEC UPDATED / PARTIAL-FUTURE; History + Post-Visit Feedback DONE / staging-smoke-passed | `docs/GROWTH_RETENTION.md`, account/history/favorites baselines where present | History and Feedback stay in regression; recommended next block is Guest Favorites Phase 1. Repeat template, simple venue promotions and opt-in notifications remain future; Platform feedback dashboard, automated review prompts, promo codes, loyalty, referrals, paid placement boosting and segmentation remain future |
...
20. Owner invite from platform can be accepted end-to-end.
