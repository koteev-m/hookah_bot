...
>
> Current docs checkpoint as of 2026-07-21: Guest growth/retention is consolidated in `docs/GROWTH_RETENTION.md`. Status remains `SPEC UPDATED / PARTIAL-FUTURE` overall, while Guest History Foundation and Post-Visit Feedback MVP are DONE / MVP / staging-smoke-passed and Guest Favorites Phase 1 is DONE / MVP / LOCAL SMOKE PASSED for venue favorites only. Catalog/detail actions, Account list, current-user isolation, unavailable-venue filtering and the shared Bot/Mini App source are closed locally. Favorite menu items, recommendations/frequent items, repeat templates, simple venue promotions and notifications remain future; Platform feedback dashboard, automated review prompts/public review automation, promo codes, loyalty stamps/points, referrals, segmentation and paid placement/boosting remain future.
>
...
- Guest growth/retention beyond the staging-smoked History/Feedback and local-smoke-passed venue favorites slices: favorite menu items, recommendations/frequent items, `REPEAT_TEMPLATE`, simple `VENUE_PROMOTION` and notifications. Each remaining loop stays partial/future until its own Bot + Mini App + backend evidence proves it.
...
2. **P1/P2**: Recommended next bounded runtime block is Order Session Tab Core Hardening: one active order per `table_session_id`, tab-scoped order views and privacy regression. Guest Favorites Phase 1 is already DONE / MVP / LOCAL SMOKE PASSED; repeat templates, simple venue promotions and notifications remain future.
...
3. Harden Order Session Tab Core as the next bounded runtime milestone: active-order uniqueness by `table_session_id`, tab-scoped order views and cross-user/cross-session privacy regression. Do not move directly from Favorites to repeat or promotions.

# Какие функции лучше не трогать пока

- Favorite menu items, recommendations/frequent items, promotions/referrals/repeat and notifications remain future until separately scoped. Venue favorites are closed locally; loyalty/tips/payments/Stars/crypto are not part of the next hardening block.
...
| Guest growth/retention | SPEC UPDATED / PARTIAL-FUTURE; History + Post-Visit Feedback DONE / staging-smoke-passed; Guest Favorites Phase 1 DONE / MVP / LOCAL SMOKE PASSED | `docs/GROWTH_RETENTION.md`, shared `guest_favorite_venues` source and current account/catalog/detail evidence | Keep venue favorites, History and Feedback in regression. Favorite menu items, recommendations/frequent items, repeat template, simple venue promotions and notifications remain future; next runtime recommendation is Order Session Tab Core Hardening |
...
20. Guest Favorites Phase 1: add from catalog; add from venue detail; remove; verify Account venue-only list; verify the specified empty state; verify two-user isolation; verify hidden/suspended/subscription-blocked filtering without row loss; verify Bot/Mini App shared source.
