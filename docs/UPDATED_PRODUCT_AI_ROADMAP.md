...
Текущий фокус перед пилотом: product P0/P1 закрыт по уже принятым M1-M9b.3 блокам, staging smoke, CI release validation, deploy/runbook hardening and minimal Guest Mini App browser smoke зелёные. M9a Deployment SSH Reliability Hardening is CLOSED / staging smoke passed: standard deploy remains supported, opt-in ControlMaster deploy is validated as a release-reliability workaround, and the exact SSH/network root cause remains unconfirmed. M9b Venue Working Hours and Date Exceptions Mini App Parity, M9b.1 range/rejection-copy improvements, M9b.2 exception save/list UX and M9b.3 date-range editing are CLOSED / staging smoke passed. Platform Owner Invite / ADMIN Semantics Hardening, Platform Venue OWNER Revocation and H2/PostgreSQL active-order + personal-tab uniqueness fidelity are CLOSED. Selected next bounded milestone: Mini App mutation and operational verification/fix pack for CORS, staff-call payload/notification and fallback chat order payload, because launch-critical broken guest/staff operational flows outrank billing and growth work if current code or smoke finds a regression. Scope не расширяем в сторону Telegram-native AI surfaces до готовности Mini App и public-safe tools.
...
1. Mini App mutation and operational fix pack: CORS, staff-call payload/notification, fallback chat order payload.
   - Why: after the database-fidelity milestone closed, launch-critical broken guest/staff operational flows outrank billing and growth. Current docs mark the historical CORS mutation, staff-call lifecycle and fallback payload issues as fixed or superseded, so this should start with focused verification and become implementation only for confirmed current regressions.
   - Acceptance: mutation preflight, staff-call create/notify/ACK/DONE and fallback chat order payload all pass current smoke; any discovered regression gets a bounded fix without reopening broad Platform Mode, lifecycle, billing or analytics work.

2. Platform Billing Cockpit / Owner Payment UX.
   - Why: platform owner onboarding/access and order/session/tab database fidelity are closed, so commercial operation is the next platform launch gap after operational smoke is protected. Billing still should not outrank broken guest/staff flows if verification finds them.
   - Acceptance: trial/paid/past-due/suspended rules are clear and visible; invoice/payment/manual mark-paid paths are operator-safe and audited where needed.

3. Guest-facing bill/display-number/full-bill parity.
   - Why: money/order presentation is a launch-critical trust surface and should stay ahead of polish, bookings growth and analytics. Existing bot/Mini App bill paths have parity automation, so this should be a focused verification/fix slice rather than a broad billing rewrite.
   - Acceptance: Guest Mini App, Venue Mini App and Telegram/staff bill snapshots show the same display number, payable total, discounts, exclusions and status copy for active and closed orders.
...
- Staff call lifecycle ACK/DONE is already CLOSED / staging smoke passed; keep it in regression.
