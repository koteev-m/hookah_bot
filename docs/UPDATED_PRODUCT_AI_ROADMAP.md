...
- Staff profiles, Today Shift, optional Staff Schedule and future staff tips are canonical in
  `docs/STAFF_PROFILES_SHIFTS_TIPS.md`: `STAFF_PROFILE + SHIFT_TODAY PHASE 1` is
  `DONE / MVP / STAGING-SMOKE-PASSED`; `STAFF_SCHEDULE PHASE 1` is implementation-ready but has no
  runtime and expects no migration; Phase 2 may add external staff tip link + `staff_tip_intent`;
...
| P1 SPEC READY / RUNTIME NOT IMPLEMENTED / NO MIGRATION EXPECTED | Staff schedule | `SHIFT_TODAY` is implemented as manual current-day Guest visibility; `staff_shifts` already has one profile/start-date constraint, local times, statuses and `updated_at`. | Optional Venue Mini App week planning: Owner/Manager bounded create/update/cancel; Staff own shifts plus safe overlapping colleagues; venue timezone/overnight, computed lifecycle, stale guard and atomic audit; no Guest auto-publication. | Implement the canonical bounded contract. Keep Today Shift/Guest unchanged and keep availability, recurring/split shifts, reminders, swaps, Telegram and payroll out of scope. |
...
- Staff Schedule now has a dedicated implementation-ready optional-module spec and is a bounded
  runtime candidate. Staff Photo Upload still needs consent plus safe media storage/picker policy
  and is not part of that implementation.
...
Next implementation-ready bounded candidate:
**STAFF SCHEDULE PHASE 1 / OPTIONAL VENUE SHIFT PLANNING**. Reuse `staff_profiles` / `staff_shifts`
without migration; implement Owner/Manager week management and Staff own/overlap read in Venue Mini
App; preserve manual Today Shift and Guest `Сегодня работают`; keep Telegram, reminders, recurring
or split shifts, payroll, staff media and Media Upload/R2 out of scope. Canonical acceptance and
validation gates are in `docs/STAFF_PROFILES_SHIFTS_TIPS.md` and
