...
| P1 IMPLEMENTED / NEEDS STAGING SMOKE | Guest table session persistence/restore | Backend now has authenticated `GET /api/guest/table/restore`; Mini App startup restores the latest safe active table context when no explicit QR token is present, and explicit QR/start token still wins. Automated coverage includes active restore, cross-user denial, closed-only denial, latest-context selection, browser startup restore and account-switch storage isolation. | While an active table session/tab/order exists, returning guest should re-enter table context safely without rescanning QR; after bill close, table context can reset. | Deploy and run focused staging smoke before marking P1 closed. Keep authorization/session isolation checks in regression coverage. |
...
Recommended next block: focused staging smoke for `P1 Guest table session persistence/restore`, because the code path is implemented locally but must be verified in real Telegram WebApp runtime before marking P1 closed.
