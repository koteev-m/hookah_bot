...
- Guest/Menu Options & Flavors parity now covers structured selected-option persistence, Guest Bot/Mini App option choice, item-scoped Venue Mini App option CRUD, and explicit item/option stop-list controls. Remaining follow-ups: Guest Bot line preference-note input and Mini App base flavor profile batch/normalize helper.
...
- Venue Mini App base flavor profile batch/normalize helper after a shared item-scoped template API; preserve STAFF no-settings/no-menu-management boundaries;
- venue stats screen;
...
| P1 ACTIVE | Guest/Menu Options & Flavors Parity | Backend selected-option persistence, Guest Bot structured submit, Guest Mini App option picker/cart identity, Venue Mini App item-scoped option CRUD, and explicit item/option stop-list controls are implemented by slices. | Menu item options/flavors are structured order modifiers. Guest Bot and Guest Mini App both show/select them; same item with different selected option or line-level `Пожелание к вкусу` is a distinct cart/order line; selected options affect price; line preference notes do not affect price; venue order detail, guest active order/bill, Guest Bot `Мой заказ` and staff chat show selected options/notes; unavailable/stop-listed options are hidden or rejected. | Remaining slices: add Guest Bot input parity for optional line note; add shared base flavor profile template/batch API before exposing Mini App `Добавить базовые вкусы` / normalize helper; keep smoke/docs closure. |
