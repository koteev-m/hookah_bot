...
| `CATALOG-MAP-VENUE-LINKS-001` — automatic canonical link per venue and Guest catalog map | `ЗАПЛАНИРОВАНО / AFTER V126 CUTOVER` | [`CATALOG_MAP_AND_VENUE_LINKS`](CATALOG_MAP_AND_VENUE_LINKS.md); Product Spec, Growth, Platform, Security | Catalog search and nullable venue coordinates exist, but public venue identity/resolver, owner link/QR UI and map do not. Create links automatically for every venue, then coordinate verification and a provider-separated MapLibre view; no per-venue code/config. | Venue sharing and Guest discovery / manual pilot links do not scale and unverified provider assumptions create licensing/privacy risk. | V126 cutover complete; domain/provider/slug/privacy decisions. | L/XL / YES; docs-first, then DB/API/Venue/Platform/Guest; PG+F+E2E+A+S; ordered slices in canonical document; P2 planned. |
| `GROWTH-CATALOG-SCALE-001` — large-catalog discovery and ranking | `DEFERRED_AFTER_MVP` | MRWI; Growth, QA, Product Spec | Search/filter Phase 1 is staging-closed on limited data. Pagination/ranking and large-pilot behavior remain; map/geo is separately planned in `CATALOG-MAP-VENUE-LINKS-001`, and environment smoke is separately tracked. | Guest discovery / degraded results at scale. | Representative dataset and ranking decision. | L / POSSIBLE; catalog queries/API/UI; performance F+A+S; at dataset growth; P2/P3. |
