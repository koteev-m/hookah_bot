...
- Staff profiles, Today Shift, optional Staff Schedule and future staff tips are canonical in
  ...
  deploy, PostgreSQL V120/H2 V121 invite-revoke rollout and manual smoke are complete. Optional
  module/source settings are the next separate Slice B. Phase 2 may add
...
| STAFF OPERATIONS SLICE A / MANAGER PARITY + SHIFT TIME DEFAULTS / DONE / MVP / STAGING-SMOKE-PASSED | Staff access + team cards + shift defaults | Manager Staff-only invite create/list/revoke and display-only/Staff-linked card management are implemented with protected Owner/Manager linkage and transaction-bound safe audit. | Schedule GET returns batched effective venue hours; create AUTO defaults, MANUAL preservation and edit stored-time invariant are implemented. | Keep invite revocation, protected profiles, effective-hours defaults and role isolation in regression. Optional module settings and Guest source are separate Slice B work. |
...
| NEXT STAFF BLOCK / IMPLEMENT_STAFF_OPERATIONS_SLICE_B_NOW | Optional Team and Schedule Module / Guest MANUAL or SCHEDULE source | Read-only runtime audit at HEAD `4ae93e810ffa8d72532244e48963a8a510b0adfa` found the reusable `venue_settings.updated_at`, shared Guest/Preview read service and schedule lifecycle domain. Required columns are absent; current heads are PostgreSQL V120/H2 V121, so the exact next files are PostgreSQL V121 and H2 V122 after recheck at implementation start. | Add backward-compatible master/Guest visibility/source settings, narrow Owner/Manager authority, per-route fail-closed guards, retained-data re-enable, persisted MANUAL Today switch and active-only timezone/overnight SCHEDULE resolver with no fallback or private schedule data. | Exactly one next Staff runtime block. Full implementation-ready contract, tests, rollout smoke and outcome-first prompt are canonical in `docs/STAFF_PROFILES_SHIFTS_TIPS.md`. Payroll, attendance, recurring shifts, swaps, reminders, Telegram mutation UI, staff chat, media/R2, tips/payments, staff membership mutation, Guest future schedule, automatic deletion and campaigns stay out of scope. |
...
Shift and Guest `Сегодня работают` remain unchanged. Green Actions, runtime deploy, Slice A V120/H2
V121 rollout and manual smoke are complete. The separate free-member create-from-member manual
scenario remains non-blocking in `STAFF-IDENTITY-MANUAL-001`. Optional module settings and Guest
source changes are selected as Slice B; Telegram, reminders, recurring/split shifts, mass edit, payroll, staff
