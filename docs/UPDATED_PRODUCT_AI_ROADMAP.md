...
- Venue Mini App Guest Preview Phase 2.1 is **VENUE MINI APP GUEST PREVIEW / PUBLISHED + PRIVATE DRAFT READ-ONLY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED**. One `Предпросмотр для гостя` entry calls one Venue endpoint; the server returns `PUBLISHED_PUBLIC` only through the unchanged Guest DTO/read assembly and availability/subscription guards, otherwise it returns OWNER/MANAGER own-venue `PRIVATE_DRAFT` with an allowlisted projection of saved guest-facing state. STAFF/foreign access is denied, Guest routes remain closed to unpublished state, responses are `no-store`, scoped preview media is authenticated without raw refs, inactive/hidden child data and guest mutation controls are absent, settings dirty-state and venue-switch stale-response guards are covered. Focused backend preview/Guest/RBAC/promotion tests, Kotlin compile/lint, Mini App build and deterministic browser smoke `95/95` passed locally; CI and staging smoke remain required.
...
15. Venue Mini App Guest Preview Phase 2.1: **VENUE MINI APP GUEST PREVIEW / PUBLISHED + PRIVATE DRAFT READ-ONLY / MVP IMPLEMENTED / LOCAL VALIDATION PASSED**. One OWNER/MANAGER own-venue entry `Предпросмотр для гостя` uses a server-selected mode: `PUBLISHED_PUBLIC` is the exact guarded Guest venue/info projection, while `PRIVATE_DRAFT` is the saved public-facing projection for a venue not currently guest-visible. The UI shows exact published/private badges and copy, safe lifecycle reasons, origin-aware return, authenticated scoped existing media and no guest actions. Unsaved public-card, weekly-schedule or date-exception edits block preview with no auto-save; venue switch aborts/clears and rejects late state. STAFF/foreign/Platform-only access, private fields, hidden sections, unpublished staff, inactive/non-current promotions and raw refs remain denied. CI and staging smoke remain open; media upload, draft editing, publication and share links remain out of scope.
