...
- Venue Mini App Published Guest Preview Phase 1 is **MVP IMPLEMENTED / LOCAL VALIDATION PASSED** for OWNER/MANAGER on the unchanged shared Guest DTO/read assembler and its lifecycle/subscription guards; STAFF/foreign access is denied, unavailable venues fail closed, and the screen has no guest mutations or table/cart/order dependency. CI and staging smoke remain pending.
- Venue Mini App Draft Preview Phase 2.1 is **MVP IMPLEMENTED / LOCAL VALIDATION PASSED**: OWNER/MANAGER can open the same read-only `Предпросмотр карточки` screen for saved `DRAFT` state through a separate Venue API/read model. STAFF/foreign access is denied; Guest APIs are not weakened; HIDDEN/PAUSED/SUSPENDED/ARCHIVED/DELETED projections, unsaved changes, publish actions and share links remain excluded. Draft visible info sections are text-only with one post-publication media hint and no media refs/routes; Published media is unchanged. CI and staging smoke remain pending.
...
15. Venue Mini App Published Guest Preview Phase 1: **MVP IMPLEMENTED / LOCAL VALIDATION PASSED**. OWNER/MANAGER use the same guest-visible `VenueResponse` and info-section read assembly as Guest, including weekly hours, future date exceptions, public media/info, Today Staff and current active promotions. STAFF and foreign venue access are denied; unavailable venues return safe copy; booking/favorites/chat/support/staff-call/extension/cart/order/table context, mutations and unpublished content are excluded. CI and staging smoke remain pending.
16. Venue Mini App Draft Preview Phase 2.1: **MVP IMPLEMENTED / LOCAL VALIDATION PASSED**. A separate OWNER/MANAGER-only Venue endpoint/read model exposes saved `DRAFT` public-candidate fields, visible text info sections, guest-visible Today Staff and current `ACTIVE` promotions without weakening Guest availability guards. Draft media refs/routes are excluded and one compact post-publication hint is shown; Published media is unchanged. Non-DRAFT private preview, mutations, unsaved changes, publish workflow and share links remain out of scope. CI and staging smoke remain pending.
