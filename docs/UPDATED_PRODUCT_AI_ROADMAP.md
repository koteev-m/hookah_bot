...
- Staff profiles, today shift and future staff tips are canonical in `docs/STAFF_PROFILES_SHIFTS_TIPS.md`: Phase 1 is `STAFF_PROFILE + SHIFT_TODAY` with no payments; Phase 2 may add external staff tip link + `staff_tip_intent`; provider/direct payout, Telegram Stars and crypto are not MVP.
...
| NEXT / SPEC READY | Staff profiles + today on shift | Canonical spec exists in `docs/STAFF_PROFILES_SHIFTS_TIPS.md`; no runtime implementation yet. | Owner creates/edits/publishes opt-in public staff profiles, profiles may be linked to venue members or display-only, Owner/Manager marks `Сегодня на смене` if policy allows, and guests see only public visible profiles/shifts. | Implement Phase 1 only: no tips payments, no provider, no Stars, no crypto, no guest order online payment. Keep `linked_user_id` private and add role-denial/audit coverage. |
...
2. Guest growth / retention MVP.
   - Why: retention features are valuable after stable operations and billing, but they depend on a trustworthy visit/order/session model and opt-in notification rules.
   - Acceptance: closed visits/orders/bookings are visible in guest history, favorites can be managed safely, repeat actions create templates that require the next table context and current menu/stop-list validation, post-visit feedback is requested only after confirmed visit, simple promotions have terms/period/status, and promo notifications require opt-in.
...
Recommended next bounded milestone: Implement Phase 1: Staff profiles + today on shift, no payments.

Why:
- support/tickets MVP and guest communication routing are now backend-backed, role-scoped and smoke-tested;
- manual billing and renewal/courtesy are closed, but guest order payment still remains offline through the venue terminal;
- staff visibility is a bounded venue/guest read-model slice and does not require choosing a payment provider;
- staff tips remain future until external-link intent, moderation and legal/payment decisions are specified separately.
