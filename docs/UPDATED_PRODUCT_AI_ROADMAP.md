...
| P1 IN PROGRESS | Paid venue/shift extension | Backend data/API, Guest Mini App request UX, bill service charges and Venue Mini App owner/manager settings are implemented. Current Venue Mini App approve/reject UX still lives in a standalone `Продления` section, while the product target is order/table detail integration. Existing `order_batch_items` require `menu_item_id`, so extension remains a separate service charge rather than a normal menu/cart item. Guest Bot, Owner/Manager Bot settings, and pending-request staff-chat live message actions are still pending. | Guest requests extension from active table context through service action `Продление работы заведения` in Mini App and bot table flow; STAFF/MANAGER see and approve/reject fixed-price requests inside active order/table/bill context and staff-chat live order message; MANAGER/OWNER configure price/duration in Mini App and bot; approval adds a dedicated service charge and extends the active table/session orderable window. | Implement in small slices: A) backend order-scoped pending extension read model + Venue Mini App order queue/detail integration and demote `Продления`; B) staff-chat live message pending block with inline approve/reject, no noisy new messages; C) Guest Bot table-flow request entry; D) Owner/Manager Bot settings parity; E) regression smoke/docs closure. Preserve STAFF no-settings rule and never expose extension as catalog item/cart item/order batch item. |
...
Recommended next block: implement `P1 Paid venue/shift extension` slice A: backend order-scoped pending extension read model plus Venue Mini App order queue/detail integration, then demote the standalone `Продления` island.
