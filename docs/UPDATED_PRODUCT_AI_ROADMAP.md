...
- M5 staff calls lifecycle is CLOSED after staging smoke: Guest Mini App uses a transient staff-call modal and compact status card, Venue Mini App has a real `Вызовы` queue with accept/close, backend/staff-chat callbacks share the same lifecycle, and ACK/DONE audit hardening is CLOSED / staging smoke passed across Venue Mini App and Telegram staff-chat surfaces. Keep notification delivery plus actor audit evidence in regression. A completed DONE staff-call card may visually linger for too long; treat dismiss-after-DONE behavior as a separate UX follow-up unless it regresses session exit or repeated staff calls.
...
1. Guest-facing bill/display-number/full-bill parity.
   - Why: money/order presentation is a launch-critical trust surface and should stay ahead of platform billing, bookings growth and support. After table-context exit is closed, the next guest risk is whether Mini App and Bot show the same human order/bill identity, totals, discounts, exclusions and closed-state copy.
   - Acceptance: Guest Mini App, Venue Mini App and Telegram/staff bill snapshots show the same display number, payable total, discounts, exclusions, service charges, active/closed status copy and no technical order ids in guest-facing UI.

2. Venue Mini App full bill / discounts / excluded items parity.
   - Why: venue staff/manager money controls are the operational counterpart to guest bill trust. This should be a focused parity and regression slice over existing backend-owned bill snapshots, not a billing/payment redesign.
   - Acceptance: Venue Mini App order detail matches Telegram/staff bill for manual discounts, promo/loyalty discounts, excluded/restored items, service charges, close state and STAFF manager/owner permissions.

3. Completed staff-call card UX cleanup / dismiss-after-DONE behavior.
   - Why: the lingering `Вызов выполнен...` card is a verified UX follow-up, but staging showed it does not block table exit and DONE calls do not block new session exit. It is small and useful, but should not outrank order/bill correctness.
   - Acceptance: DONE status is visible long enough to confirm completion, then no longer visually dominates table context; repeated staff-call creation remains allowed only after DONE and still prevents duplicate NEW/ACK calls.
...
- Platform Billing Cockpit / Owner Payment UX stays important, but should wait behind guest/staff bill correctness unless a concrete launch-commercial blocker appears.
