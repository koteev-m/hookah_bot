...
- Venue Mini App booking card opens a persisted booking conversation thread: venue messages, Guest Bot replies and Guest Mini App replies share one source of truth; staff chat remains a notification mirror. M4A staging smoke passed after UX polish. M4B unified `Сообщения` inbox is implemented locally as a smoke target: thread cards show context/status/last message/unread and support active/resolved filters. M4C resolve/reopen lifecycle is implemented locally: conversations can move between active and resolved without changing booking status.
...
- M4B Unified Messages Inbox UX is implemented locally as the next smoke target: Guest and Venue Mini App thread lists now use backend context labels, status, last message preview/time, unread counts via `support_thread_reads`, and `Активные` / `Завершённые` filters.
- M4C support thread resolve/reopen lifecycle is implemented locally: Guest and Venue Mini App can `Завершить переписку`, resolved threads move to `Завершённые`, `Возобновить переписку` moves them back to `Активные`, and sending a real user message to a resolved thread reopens it through shared backend message semantics.
...
- M4B Unified Messages Inbox UX implemented locally / smoke target: multi-venue guest inbox, venue-scoped inbox, context/status/last-message/unread cards and active/resolved filters;
...
5. M4B: Unified Messages Inbox UX; Status: implemented locally / smoke target. Guest `Сообщения` / `Мои обращения` is a thread list with venue, context, status, last message, time and unread badge; Venue `Сообщения` is scoped to the current venue; Platform Support Center remains later and only backend-backed.
6. M4C: Support thread resolve/reopen lifecycle; Status: implemented locally / smoke target. Conversation lifecycle is separate from booking lifecycle; active/resolved filters now have explicit user actions.
7. M5: Staff Calls Lifecycle and Notification Parity; Status: implemented locally / staging smoke target. Guest Mini App creates scoped staff calls and shows simple status; Venue Mini App accepts/closes active calls; staff chat remains notification mirror.
8. M6: read-only promotions/growth summary; builders stay bot-canonical until explicit Mini App APIs/RBAC exist.
9. M7: guest preview entry using guest-visible read models only.
10. M8: small venue settings slices, not one bulk settings endpoint.
11. M9: staff chat diagnostics/unlink polish.
...
| --- | --- | --- | --- | --- |
| P1 ACTIVE | Venue Bot-to-Mini-App Parity Program | Bot selected-venue hub already has sections `Работа смены`, `Настройка заведения`, `Статистика`, `Продвижение`, `Предпросмотр для гостя`; Venue Mini App has M1 IA shell, M2 read-only `Статистика`, M3 booking queue/lifecycle, M4A booking conversation threads closed after staging smoke, M4B/M4C inbox lifecycle implemented locally, and M5 staff-call lifecycle implemented locally. Full map is in `docs/audit/VENUE_BOT_TO_MINIAPP_PARITY_PROGRAM.md`. | Venue Mini App should match implemented bot capabilities where practical while hiding missing/placeholder functionality. Bot and Mini App are two clients over one backend; users choose by convenience, not missing critical venue functions. | Stage-smoke M4B/M4C inbox filters/cards/unread/resolve/reopen and M5 staff-call guest create/status + venue accept/close + staff chat notification behavior. Keep structured reschedule proposals, general tickets, Platform Support Center, `Продвижение` and `Предпросмотр` out of nav until real routes/screens exist. |
...
Recommended next step: stage-smoke M4B Unified Messages Inbox UX. In parallel, complete `P1 Paid venue/shift extension` Owner/Manager Bot settings parity and regression smoke. Do not reopen the closed options/flavors, stats, booking lifecycle or M4A conversation slices unless new smoke/code evidence contradicts the current status.
...
Latest implemented parity block: M4C support thread resolve/reopen lifecycle for Guest and Venue Mini App.

Status: implemented locally / smoke target. Guest and Venue Mini App now render `Сообщения` as thread-card inboxes with context labels, status, last message preview/time, unread badges backed by `support_thread_reads`, and `Активные` / `Завершённые` filters. M4C adds explicit `Завершить переписку` / `Возобновить переписку` actions so resolved threads move out of active inbox without touching booking confirm/change/cancel/arrival lifecycle. Booking card `Открыть переписку` still opens the related thread.

Next practical step: stage-smoke M4B/M4C together, then keep follow-ups separate: structured booking reschedule, general problem tickets, venue/admin bot inbox and Platform Support Center. Do not restart promotions, growth, loyalty, advanced analytics or Telegram-native AI scope until the support inbox model is stable.
