...
- broad venue settings are not exposed as a dead-end placeholder; Venue Mini App `Настройки` now contains backend-backed booking hold, paid shift-extension settings and M8a public card basics (`city`, `address`, `guestContact`, `cardDescription`);
...
12. M8a: Venue Mini App public profile/card settings parity; Status: implemented, code/test/e2e-backed, staging smoke pending. OWNER/MANAGER can edit existing guest-facing `city`, `address`, `guestContact` and `cardDescription`; venue name is read-only; STAFF is hidden/forbidden; guest public read models reflect saved fields; no migration was required.
