...
| --- | ---: |
| `OPEN_CONFIRMED` | 41 |
| `MVP_IMPLEMENTED_LOCAL_VALIDATION_PASSED` | 2 |
...
| **Canonical catalog items below** | **107** |
...
Independent review registry for the bounded booking-integrity implementation:
...
| `BOOKING-CI-FLOOR-001` | `DONE` | Existing workflow uses exact FQCN selectors and JUnit XML files with factual minima, including legacy/strict outbox, message/PID-lock and real migration-lock suites; missing/below-minimum/skipped/failed/errored reports fail. Full Playwright uses a structured result and requires at least `206` executed with zero skipped/failures. Docker remains mandatory. |
...
| `BOOKING-MINIAPP-OUTBOX-001` | `DONE` | Every Mini App booking reply has a persisted scope-bound opaque identity; message, thread state and canonical notification outbox row use one JDBC transaction. Strict booking replay/collision remains separate from legacy key-only enqueue. Exact route, PostgreSQL `11/11` and Playwright floor `206` cover replay, mismatch, exact PID contention, failure/recovery, lost-response reload reconciliation and conflicting pre-existing envelopes. |
...
| `BOOKING-READ-LOCK-ORDER-001` | `DONE` | All 15 standalone and four connection-aware read-marker paths use `bookings -> support_threads -> support_thread_reads` or `support_threads -> support_thread_reads`; exact H2 `11/11` and independent-PID PostgreSQL `3/3` suites prove RBAC preservation, parent-before-child locks and no reverse runtime DML path. |
...
| `BOOKING-CI-PLAYWRIGHT-FLAKE-001` | `OPEN` | An earlier structured run executed `197/198` after one unrelated favorite-test failure. This pass first executed `205/206` after one unrelated catalog-debounce timing failure; its focused rerun and second full `206/206` run passed. Trigger the next Mini App CI-hardening pass or a repeated same failure in GitHub Actions; green reruns do not erase the evidence. |
