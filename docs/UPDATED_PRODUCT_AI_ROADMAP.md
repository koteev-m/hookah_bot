...
11. M7c adaptive transactional booking reminders: Status: implemented, code/test/e2e-backed, staging acceptance pending. Runtime remains opt-in disabled by default; only explicit `BOOKING_REMINDER_WORKER_ENABLED=true` starts the worker. M7c uses explicit confirmation/reschedule anchors, one scheduled pre-visit reminder maximum per booking, `QUEUED` after outbox enqueue, and Mini App guest/venue attendance indicators. Do not mark CLOSED until real Telegram smoke passes.
...
Current implementation step after the M7c code slice: M7b Guest Mini App `Мои брони` is implemented with local validation and staging backend/DB evidence, but remains `PASS_WITH_UNVERIFIED_RUNTIME_GAPS` until real Bot `/my` side-by-side and real two-account isolation smoke are completed. M7c adaptive reminders are implemented locally and covered by backend tests plus Mini App build/e2e, but are not CLOSED until real Telegram staging smoke passes. The worker remains opt-in disabled by default and must not be enabled on staging except for an approval-gated smoke. Do not reopen closed options/flavors, stats, booking lifecycle, M4A-M4C conversations, M5 staff calls, M6 staff-chat management or M7a hold settings unless new smoke/code evidence contradicts the current status.
...
M7c adaptive transactional booking reminders are implemented locally and awaiting staging acceptance.
...
- Acceptance: keep M7c open until a real Telegram staging smoke verifies one visible reminder, buttons, attendance indicators and reschedule/cancel behavior.
