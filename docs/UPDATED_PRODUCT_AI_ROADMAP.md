...
| P1 DESIGN READY / NEXT IMPLEMENTATION | Paid venue/shift extension | Product/API design is now documented in `docs/PRODUCT_SPEC.md`. Current code has table session expiry, venue hours/timezone, order batches, staff calls, RBAC and bill snapshots, but no dedicated extension domain. Existing `order_batch_items` require `menu_item_id`, so extension must be a separate service charge rather than a normal menu item. | Guest requests extension from active table context; STAFF/MANAGER can approve/reject fixed-price extension operationally; MANAGER/OWNER configure price/duration; approval adds a dedicated service charge to the bill and extends the active table/session orderable window. | Implement minimal data/API slice first: settings, request, service charge, session extension, backend tests and bill snapshot integration. Do not expose extension in structured order menu. |
