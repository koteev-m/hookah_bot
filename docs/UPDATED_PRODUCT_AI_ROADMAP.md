...
Текущий фокус перед пилотом: product P0/P1 закрыт по уже принятым M1-M9b.3 блокам, staging smoke, CI release validation, deploy/runbook hardening and minimal Guest Mini App browser smoke зелёные. M9a Deployment SSH Reliability Hardening is CLOSED / staging smoke passed: standard deploy remains supported, opt-in ControlMaster deploy is validated as a release-reliability workaround, and the exact SSH/network root cause remains unconfirmed. M9b Venue Working Hours and Date Exceptions Mini App Parity, M9b.1 range/rejection-copy improvements, M9b.2 exception save/list UX and M9b.3 date-range editing are CLOSED / staging smoke passed. Platform Owner Invite / ADMIN Semantics Hardening and Platform Venue OWNER Revocation are CLOSED / staging smoke passed. Selected next bounded implementation milestone: H2/PostgreSQL active-order + personal-tab uniqueness fidelity, because active order/session/tab correctness is launch-critical and the remaining documented risk is release-test fidelity rather than broad product redesign. Scope не расширяем в сторону Telegram-native AI surfaces до готовности Mini App и public-safe tools.
...
1. H2/PostgreSQL active-order + personal-tab uniqueness fidelity.
   - Why: active order/session/tab correctness is launch-critical. Current docs say the old runtime P0 is fixed, but PostgreSQL partial unique constraints are still not faithfully represented by H2 tests, which can hide regressions before release.
   - Acceptance: local/test migrations enforce equivalent uniqueness semantics or focused tests cover the PostgreSQL behavior directly; active order per table session and personal-tab uniqueness cannot silently regress.

2. Platform Billing Cockpit / Owner Payment UX.
   - Why: platform owner onboarding and access management are now closed, so commercial operation becomes the next platform launch gap. Billing should not outrank core order/session/tab correctness, but it matters before paid pilots.
...
3. Mini App mutation and operational fix pack: CORS, staff-call payload/notification, fallback chat order payload.
   - Why: these were historical P0/P1 launch risks. Current docs mark CORS mutation methods, staff-call lifecycle and fallback payload issues as fixed/superseded, so this should start as focused verification and become implementation only if current smoke/code proves a regression.
   - Acceptance: mutation preflight, staff-call create/notify/ACK/DONE and fallback chat order payload all pass current smoke; any discovered regression gets a bounded fix without reopening broad Platform Mode or lifecycle work.

Not selected as implementation right now:
- Broad Guest Order / Table Session / Tab Scoping Hardening is not reopened without new evidence; the remaining documented risk is H2/PostgreSQL fidelity.
