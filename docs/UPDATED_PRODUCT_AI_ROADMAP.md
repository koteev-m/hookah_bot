...
10. M7b: Guest Mini App `Мои брони`; Status: implemented, code/test/e2e-backed, staging audit verdict `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`. Remaining runtime checks are real Bot `/my` versus Guest Mini App side-by-side comparison and real two-account booking isolation.
11. M7c safety gate: legacy booking reminder runtime is opt-in disabled by default. Status: implemented. Adaptive M7c reminders are still pending and not accepted.
12. M8: read-only promotions/growth summary; builders stay bot-canonical until explicit Mini App APIs/RBAC exist.
13. M9: guest preview entry using guest-visible read models only.
14. M10: menu semantic type/media polish after current options/flavors regression remains green.

| Priority | Block | Current evidence | Product target | Recommended action |
| --- | --- | --- | --- | --- |
| P1 ACTIVE | Bot-to-Mini-App Parity Program | Bot selected-venue hub already has sections `Работа смены`, `Настройка заведения`, `Статистика`, `Продвижение`, `Предпросмотр для гостя`; Venue Mini App has M1 IA shell, M2 read-only `Статистика`, M3 booking queue/lifecycle, M4A booking conversation threads, M4B/M4C inbox lifecycle, M5 staff-call lifecycle, M6 staff-chat diagnostics/unlink and M7a booking hold settings closed. Guest Mini App M7b `Мои брони` is implemented with code/test/e2e evidence and staging audit verdict `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`; legacy reminder runtime is gated off by default and not accepted as M7c. Full map is in `docs/audit/VENUE_BOT_TO_MINIAPP_PARITY_PROGRAM.md`. | Bot and Mini App are two clients over one backend; required core surfaces must be aligned or documented as intentional exceptions. Mini App should show only backend-backed functionality. | Complete the remaining manual M7b runtime checks as release QA, then implement M7c adaptive transactional booking reminders as a separate accepted runtime slice. Keep structured venue-initiated reschedule proposals, general tickets, Platform Support Center, `Продвижение` and `Предпросмотр` out of nav until real routes/screens exist. |
...
Current implementation step after the M7c safety checkpoint: M7b Guest Mini App `Мои брони` is implemented with local validation and staging backend/DB evidence, but remains `PASS_WITH_UNVERIFIED_RUNTIME_GAPS` until real Bot `/my` side-by-side and real two-account isolation smoke are completed. M7c adaptive reminders are not accepted; legacy reminder runtime is opt-in disabled by default and must not be enabled before dedicated implementation, data reconciliation and Telegram staging smoke. Do not reopen closed options/flavors, stats, booking lifecycle, M4A-M4C conversations, M5 staff calls, M6 staff-chat management or M7a hold settings unless new smoke/code evidence contradicts the current status.
...
Latest safety block: M7c booking reminder safety gate.

M7b status: IMPLEMENTED with code/test/e2e evidence and staging audit verdict `PASS_WITH_UNVERIFIED_RUNTIME_GAPS`. Guest Mini App exposes `Профиль → Мои брони`, lists active/upcoming bookings across venues, uses backend public booking labels and venue-local display/deadline fields, and reuses existing guest change/cancel endpoints. Remaining runtime checks: compare one real booking in Bot `/my` and Guest Mini App, and verify real two-account isolation.
...
- Legacy runtime note: old `booking_reminders` tables, worker, scheduler and callbacks exist from earlier pilot work, but they do not match the adaptive M7c policy. The worker is now opt-in disabled by default (`BOOKING_REMINDER_WORKER_ENABLED=false` in examples); explicit `true` preserves legacy behavior only for controlled development/testing.
- Before future enablement: audit all legacy `PENDING`/`FAILED` rows, cancel or recalculate legacy-policy rows, implement and test final adaptive policy, run real Telegram smoke, then explicitly set the feature flag true after acceptance.
