...
- Venue Mini App Published Guest Preview Phase 1 is **DONE / MVP / STAGING-SMOKE-PASSED** for OWNER/MANAGER on the unchanged shared Guest DTO/read assembler and its lifecycle/subscription guards; exact real-Guest parity, STAFF/foreign denial, no guest mutations and venue-switch isolation passed after green Actions and staging deploy.
- Venue Mini App Draft Preview Phase 2.1 is **DONE / MVP / STAGING-SMOKE-PASSED**: OWNER/MANAGER open the same read-only `Предпросмотр карточки` screen for saved `DRAFT` state through a separate Venue API/read model. The permanent draft banner, guest-safe allowlist, private-field absence, closed Guest route, safe unsupported lifecycle statuses, both entrypoints, stale-free switching and media placeholder/no raw refs passed. The phase did not add or validate media upload.

Remaining P1:

- final staging smoke after each release batch;
- continue bounded venue settings slices where backend-backed; media-section authoring remains bot/platform-canonical, while Published Guest Preview, saved-DRAFT Preview Phase 2.1 and informational promotions now have backend-backed Venue Mini App surfaces; draft media remains text-only until publication, and venue subscription state is covered by the staging-smoked billing MVP, while real acquiring and Telegram Stars remain separate future work;
...
15. Venue Mini App Published Guest Preview Phase 1: **DONE / MVP / STAGING-SMOKE-PASSED**. OWNER/MANAGER use the same guest-visible `VenueResponse` and info-section read assembly as Guest, including weekly hours, future date exceptions, public media/info, Today Staff and current active promotions. Exact real-Guest parity, STAFF/foreign denial, no mutation actions and venue-switch isolation passed.
16. Venue Mini App Draft Preview Phase 2.1: **DONE / MVP / STAGING-SMOKE-PASSED**. A separate OWNER/MANAGER-only Venue endpoint/read model exposes saved `DRAFT` public-candidate fields, visible text info sections, guest-visible Today Staff and current `ACTIVE` promotions without weakening Guest availability guards. The permanent draft banner, private-field absence, Guest-route closure, safe unsupported statuses, both entrypoints, stale-free switching and media placeholder/no raw refs passed. Non-DRAFT private preview, mutations, unsaved changes, publish workflow and share links remain out of scope.
