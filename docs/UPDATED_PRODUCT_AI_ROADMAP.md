...
- broad venue settings are not exposed as a dead-end placeholder; `Настройки продления` is a real backend-backed screen;
...
8. M6: staff chat diagnostics/unlink polish. Expose existing owner-only unlink/status flow in Venue Mini App and keep Telegram group notification smoke explicit.
...
| --- | --- | --- | --- | --- |
| P1 ACTIVE | Venue Bot-to-Mini-App Parity Program | Bot selected-venue hub already has sections `Работа смены`, `Настройка заведения`, `Статистика`, `Продвижение`, `Предпросмотр для гостя`; Venue Mini App has M1 IA shell, M2 read-only `Статистика`, M3 booking queue/lifecycle, M4A booking conversation threads closed after staging smoke, M4B/M4C inbox lifecycle code/test-backed, and M5 staff-call lifecycle code/e2e-backed. Full map is in `docs/audit/VENUE_BOT_TO_MINIAPP_PARITY_PROGRAM.md`. | Venue Mini App should match implemented bot capabilities where practical while hiding missing/placeholder functionality. Bot and Mini App are two clients over one backend; users choose by convenience, not missing critical venue functions. | Next bounded implementation: M6 staff chat diagnostics/unlink polish. In parallel, stage-smoke M4B/M4C inbox lifecycle and M5 staff-chat Telegram runtime behavior. Keep structured reschedule proposals, general tickets, Platform Support Center, `Продвижение` and `Предпросмотр` out of nav until real routes/screens exist. |
...
Recommended next implementation step: M6 staff chat diagnostics/unlink polish in Venue Mini App, reusing existing backend/bot status/link/unlink semantics. In parallel, stage-smoke M4B/M4C inbox lifecycle and M5 staff-call Telegram runtime behavior. Do not reopen the closed options/flavors, stats, booking lifecycle or M4A conversation slices unless new smoke/code evidence contradicts the current status.
...
Latest implemented parity block: M5 staff calls lifecycle and compact Guest Mini App staff-call UX.
...
Next implementation step: M6 staff chat diagnostics/unlink polish. This should expose the already implemented staff-chat status/link/unlink backend semantics in Venue Mini App, preserve owner-only unlink, keep manager/staff boundaries explicit, and keep manual Telegram group notification smoke in the checklist. Do not restart promotions, growth, loyalty, advanced analytics or Telegram-native AI scope before this operational notification dependency is launch-safe.
