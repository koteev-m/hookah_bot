...
| P1 DONE / LOCAL SMOKE-PASSED | Staff profiles + today on shift | Phase 1 backend + Mini App implementation exists and local smoke passed; canonical status is in `docs/STAFF_PROFILES_SHIFTS_TIPS.md`. | Owner creates/edits/publishes opt-in public staff profiles, profiles may be linked to venue members or display-only, Owner/Manager marks `Сегодня на смене`, and guests see only public visible profiles/shifts. Venue staff cards are compact, create form is collapsed by default, `Другое` requires custom role name, raw User ID / Photo ref are not exposed, and guest `Сегодня работают` appears below main venue info. | Keep in regression and continue staging UX acceptance. Do not add tips payments, providers, Stars, crypto, guest order online payment, schedule, photo upload or staff chat sign-up inside Phase 1. |
