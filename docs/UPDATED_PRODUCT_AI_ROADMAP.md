...
- Venue Mini App Published Guest Preview Phase 1 baseline is **DONE / MVP / STAGING-SMOKE-PASSED** for OWNER/MANAGER on the unchanged shared Guest DTO/read assembler and its lifecycle/subscription guards. The 2026-07-30 hardening delta is **VENUE MINI APP GUEST PREVIEW / PUBLISHED READ-ONLY PHASE 1 / MVP IMPLEMENTED / LOCAL VALIDATION PASSED**: both adapter reads are `no-store`, the Published-only entry/helper/return action and safe unavailable copy are explicit, and PUBLISHED-to-PUBLISHED stale-response isolation is covered. Green Actions and staging re-smoke remain required before this delta inherits the baseline staging status.
...
15. Venue Mini App Published Guest Preview Phase 1: historical baseline **DONE / MVP / STAGING-SMOKE-PASSED**; current hardening **VENUE MINI APP GUEST PREVIEW / PUBLISHED READ-ONLY PHASE 1 / MVP IMPLEMENTED / LOCAL VALIDATION PASSED**. OWNER/MANAGER still use the same guest-visible `VenueResponse` and info-section read assembly as Guest, including weekly hours, future date exceptions, public media/info, Today Staff and current active promotions. The hardening adds non-cacheable adapter responses, exact Published-only UX/safe-unavailable copy and explicit PUBLISHED-to-PUBLISHED stale isolation; CI and staging re-smoke remain open.
